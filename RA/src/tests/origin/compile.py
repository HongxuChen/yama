...
if checkArgs(sys.argv):
    ...
    if checkIfIsAnalysis(sys.argv):
        ...
        clangCmd = "clang -c -emit-llvm " + fileName + \
                   fileExtension + " -o " + fileName + ".bc"
        print(clangCmd)
        os.system(clangCmd)
        print("Generating eSSA")
        eSSACmd = "opt -instnamer -mem2reg -break-crit-edges -load " + passPath + \
                  "vSSA.so -vssa " + fileName + ".bc -o " + fileName + ".essa.bc"
        print(eSSACmd)
        os.system(eSSACmd)
        print("Running the Range Analysis Pass")
        raCmd = "opt -load " + passPath + "RangeAnalysis.so " + \
                sys.argv[1] + " " + fileName + ".essa.bc -stats"
        print(raCmd)
        os.system(raCmd)
    elif checkIfIsTest(sys.argv):
        print("Running the Range Analysis Test Pass")
        raCmd = "opt -load " + passPath + "RangeAnalysis.so -load " + passPath + \
                "RangeAnalysisTest.so " + sys.argv[1] + " " + fileName + ".essa.bc"
        print(raCmd)
        os.system(raCmd)
    else:
        printHelp()
