...
FakeStats s_usedBits = FakeStats("Initial number of bits.");
FakeStats s_needBits = FakeStats("Needed bits.");
FakeStats s_percentReduction =
    FakeStats("Percentage of reduction of the number of bits.");
FakeStats s_numSCCs = FakeStats("Number of strongly connected components.");
FakeStats s_numAloneSCCs =
    FakeStats("Number of SCCs containing only one node.");
FakeStats s_sizeMaxSCC = FakeStats("Size of largest SCC.");
FakeStats s_numVars = FakeStats("Number of variables");
FakeStats s_numUnknown = FakeStats("Number of unknown variables");
FakeStats s_numEmpty = FakeStats("Number of empty-set variables");
FakeStats s_numCPlusInf = FakeStats("Number of variables [c, +inf].");
FakeStats s_numCC = FakeStats("Number of variables [c, c].");
FakeStats s_numMinInfC = FakeStats("Number of variables [-inf, c].");
FakeStats s_numMaxRange = FakeStats("Number of variables [-inf, +inf].");
FakeStats s_numConstants = FakeStats("Number of constants.");
FakeStats s_numZeroUses = FakeStats("Number of variables without any use.");
FakeStats s_numNotInt = FakeStats("Number of variables that are not Integer.");
FakeStats s_numOps = FakeStats("Number of operations");
FakeStats s_maxVisit =
    FakeStats("Max number of times a value has been visited.");

namespace ra {
void dumpStats() {
  std::ostream &cs = std::cout;
  cs << s_usedBits << "\n";
  cs << s_needBits << "\n";
  cs << s_percentReduction << "\n";
  cs << s_numSCCs << "\n";
  cs << s_numAloneSCCs << "\n";
  cs << s_sizeMaxSCC << "\n";
  cs << s_numVars << "\n";
...
IntraProceduralRA<CGT>::~IntraProceduralRA() {
  ...
  formated << 100 * (1.0 - ((double)(s_needBits.value()) / s_usedBits.value()));
  errs() << formated.str() << "\t - "
...
#endif
  // STATS
  s_numSCCs += sccList.worklist.size();
#ifdef SCC_DEBUG
  unsigned numberOfSCCs = s_numSCCs;
#endif
...
// For each SCC in graph, do the following
  ...
  for (Nuutila::iterator nit = sccList.begin(), nend = sccList.end();
    ...
    if (component.size() == 1) {
      ...
      if (component.size() > s_sizeMaxSCC.value()) {
        s_sizeMaxSCC = component.size();
      }
...
void ConstraintGraph::computeStats() {
  ...
  double totalB = s_usedBits.value();
  double needB = s_needBits.value();
  double reduction = (totalB - needB) * 100 / totalB;
  s_percentReduction = (unsigned int)reduction;

  s_numVars += this->vars.size();
  s_numOps += this->oprs.size();
}
...
void Nuutila::visit(Value *V, std::stack<Value *> &stack, UseMap *useMap) {
  ...
  for (SmallPtrSetIterator<BasicOp *> sit = (*useMap)[V].begin(),
    ...
    if (inComponent.count(name) == 0 && (dfs[root[V]] >= dfs[root[name]])) {
      root[V] = root[name];
    }
  }
