...
cl::opt<std::string> RAFilename(
    "ra-filename", cl::desc("Specify pre-computed ranges filename"),
    cl::value_desc("filename"), cl::init("stdin"), cl::NotHidden);

cl::opt<std::string> RAIgnoredFunctions(
    "ra-ignore-functions",
    cl::desc("Specify file with functions to be ignored"),
    cl::value_desc("filename"), cl::init(""), cl::NotHidden);
 ...
 * In addition, the control dependence edges have already been added to the
 *graph.
 */
void llvm::RangeAnalysis::solve() {
  ...
  for (std::list<int>::iterator SCCit = SCCorder.begin(),
                                SCCend = SCCorder.end();
       SCCit != SCCend; SCCit++) {
    int SCCid = *SCCit;
...
Range llvm::RangeAnalysis::evaluateNode(GraphNode* Node) {
   ...
   */
  else if (isa<MemNode>(Node) || isa<SigmaOpNode>(Node) ||
           isa<PHIOpNode>(Node) ||
           (isa<OpNode>(Node) &&
            dyn_cast<OpNode>(Node)->getOpCode() == Instruction::PHI)) {
    result = getUnionOfPredecessors(Node);
  }
  /*
   ...
   */
    ...
    result = abstractInterpretation(out_state[Op1], out_state[Op2],
                                    BOP->getBinaryOperator());
  }
  /*
 ...
 */
Range llvm::RangeAnalysis::abstractInterpretation(Range Op1, Range Op2,
                                                  Instruction* I) {
 ...
 *
 * Finally, if the out_state is changed, we add to the worklist the successor
 *nodes.
 */
void llvm::RangeAnalysis::computeNode(GraphNode* Node,
                                      std::set<GraphNode*>& Worklist,
                                      LatticeOperation lo) {
  Range new_out_state = evaluateNode(Node);

  bool hasChanged =
      (lo == loJoin ? join(Node, new_out_state) : meet(Node, new_out_state));
 ...
 */
void llvm::RangeAnalysis::addSuccessorsToWorklist(
    GraphNode* Node, std::set<GraphNode*>& Worklist) {
...
bool llvm::RangeAnalysis::meet(GraphNode* Node, Range new_abstract_state) {
  ...
  if (SigmaOpNode* Sigma = dyn_cast<SigmaOpNode>(Node)) {
    if (branchConstraints.count(Sigma) > 0) {
      out_state[Node] =
          out_state[Node].intersectWith(branchConstraints[Sigma]->getRange());
    }
  }
  ...
  while (it.hasNext()) {
    if (SigmaOpNode* Node = dyn_cast<SigmaOpNode>(it.getNext())) {
      if (branchConstraints.count(Node) > 0) {
        if (SymbInterval* SI =
                dyn_cast<SymbInterval>(branchConstraints[Node])) {
...
void llvm::RangeAnalysis::addConstraints(
  ...
  for (Cit = constraints.begin(), Cend = constraints.end(); Cit != Cend;
       Cit++) {
    const Value* CurrentValue = Cit->first;

    // For each value, we iterate through its uses, looking for sigmas. We
    // can only learn with conditionals when we insert sigmas, because they
    // split the
    // live ranges of the variables according to the branch results.

    Value::const_use_iterator Uit, Uend;
    for (Uit = CurrentValue->use_begin(), Uend = CurrentValue->use_end();
         Uit != Uend; Uit++) {
      const User* U = *Uit;
      if (!U) continue;

      if (const PHINode* CurrentUse = dyn_cast<const PHINode>(U)) {
        if (SigmaOpNode* CurrentSigmaOpNode =
                dyn_cast<SigmaOpNode>(depGraph->findOpNode(CurrentUse))) {
          const BasicBlock* ParentBB = CurrentUse->getParent();

          // We will look for the symbolic range of the basic block of this
          // sigma node.
          std::list<ValueSwitchMap*>::iterator VSMit, VSMend;
          for (VSMit = Cit->second.begin(), VSMend = Cit->second.end();
               VSMit != VSMend; VSMit++) {
            ValueSwitchMap* CurrentVSM = *VSMit;
            int Idx = CurrentVSM->getBBid(ParentBB);
            if (Idx >= 0) {
              // Save the symbolic interval of the opNode. We will use it in the
              // narrowing
              ...
              // If it is a symbolic interval, then we have a future value and
              // we must
              // insert a control dependence edge in the graph.
              if (SymbInterval* SI = dyn_cast<SymbInterval>(BI)) {
                if (GraphNode* FutureValue =
                        depGraph->findNode(SI->getBound())) {
...
void llvm::RangeAnalysis::addIgnoredFunction(std::string FunctionName) {
  if (!ignoredFunctions.count(FunctionName))
...
void llvm::RangeAnalysis::computeStats() {
  for (DepGraph::iterator It = depGraph->begin(), Iend = depGraph->end();
       It != Iend; It++) {
    GraphNode* Node = *It;
    ...
    assert(out_state.count(Node) &&
           "Node not found in the list of computed ranges.");
...
void llvm::RangeAnalysis::fixPointIteration(int SCCid, LatticeOperation lo) {
  ...
  if (lo == loJoin) {
    ...
    if (current_size == 1)
      numAloneSCCs++;
    else if (current_size > sizeMaxSCC)
      sizeMaxSCC = current_size;

    for (std::list<GraphNode*>::iterator it = currentSCC.begin(),
                                         iend = currentSCC.end();
         it != iend; it++) {
...
Range llvm::RangeAnalysis::getRange(Value* V) {
  if (GraphNode* Node = depGraph->findNode(V)) return out_state[Node];
...
bool IntraProceduralRA::runOnFunction(Function& F) {
  ...
  std::map<const Value*, std::list<ValueSwitchMap*> > constraints =
      brAnalysis.getIntervalConstraints();

  // Add branch information to the dependence graph. Here we add the future
  // values
...
bool InterProceduralRA::runOnModule(Module& M) {
  ...
  //BBit != BBend; BBit++){
  //
  //			for (BasicBlock::iterator Iit = BBit->begin(), Iend =
  //BBit->end(); Iit != Iend; Iit++){
  //
  //				Instruction* I = Iit;
  //
  //				Range R = getRange(I);
