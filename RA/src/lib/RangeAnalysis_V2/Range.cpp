...
Range::Range(std::string RangeString) {
  ...
  std::string ubStr =
      Contents.substr(commaPosition + 2, (cSize - commaPosition) - 2);
...
Range Range::udiv(const Range &other) {
  ...
  if (isEmpty() || other.isEmpty() || other.getUpper() == 0)
    return Range(Min, Max);
  if (other.isMaxRange()) return Range(Min, Max);
  ...
  // this could occur.
  if (Lower == Upper) return Range(Min, Max);
...
Range Range::And(const Range &other) {
  if (isEmpty() || other.isEmpty()) return Range(Min, Max, Empty);
 ...
 */
Range Range::Xor(const Range &other) { return Range(Min, Max); }
