 ...
 * I decided to write them in order to make the source code easier to
 *understand.
 *
...
class ValueBranchMap : public ValueSwitchMap {
 public:
  ...
  /// Get the "false side" of the branch
  const BasicBlock *getBBFalse() const { return getBB(0); }
  /// Get the "true side" of the branch
  const BasicBlock *getBBTrue() const { return getBB(1); }

  /// Get the interval associated to the false side of the branch
  BasicInterval *getItvF() const { return getItv(0); }
  /// Get the interval associated to the true side of the branch
  BasicInterval *getItvT() const { return getItv(1); }

  /// Change the interval associated to the false side of the branch
  void setItvF(BasicInterval *Itv) { setItv(0, Itv); }
  /// Change the interval associated to the true side of the branch
  void setItvT(BasicInterval *Itv) { setItv(1, Itv); }
};
