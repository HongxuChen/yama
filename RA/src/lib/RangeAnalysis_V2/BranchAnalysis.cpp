...
void BranchAnalysis::buildValueBranchMap(const BranchInst* br) {
  ...
  if (ConstantInt* CI = dyn_cast<ConstantInt>(ici->getOperand(1))) {
    ...
    ValueBranchMap* VBMOp0 =
        new ValueBranchMap(Op0, TBlock, FBlock, STOp0, SFOp0);
    ...
    if ((castinst = dyn_cast<CastInst>(Op0))) {
      ...
      ValueBranchMap* VBMOp1_1 =
          new ValueBranchMap(Op0_0, TBlock, FBlock, STOp1_1, SFOp1_1);
    ...
    ValueBranchMap* VBMOp1 =
        new ValueBranchMap(Op1, TBlock, FBlock, STOp1, SFOp1);
...
BranchAnalysis::~BranchAnalysis() {
  ...
  for (it = IntervalConstraints.begin(), it_end = IntervalConstraints.end();
       it != it_end; it++) {
    std::list<ValueSwitchMap*>::iterator l_it, l_end;
    for (l_it = it->second.begin(), l_end = it->second.end(); l_it != l_end;
         l_it++) {
...
static RegisterPass<BranchAnalysis> X("branch-analysis", "Branch Analysis");
