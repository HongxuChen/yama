...
static cl::opt<bool, false> TruncInstrumentation(
        "insert-trunc-checks",
        cl::desc("Insert overflow checks in the trunc instructions."),
        cl::NotHidden);

bool getTruncInstrumentation() { return TruncInstrumentation; }
    ...
    for (BasicBlock::iterator BBit = BB->begin(), BBend = BB->end();
        ...
        switch (I->getOpcode()) {
            ...
            case Instruction::Add:
                // Check if first operand is non-constant AND second operand is a
                // constant
                op = I->getOperand(0);
                ...
                if (!isa<ConstantInt>(op) && ci && (ci->getValue() != 0)) {
                    ...
                    BinaryOperator *newdef = BinaryOperator::Create(
                            Instruction::Add, op, ConstantInt::get(op->getType(), 0),
                            Twine(newname), next);
            ...
            case Instruction::Trunc:
                ...
                if (TruncInstrumentation) {
                    // Here we don't have a constant, only an interval due to the
                    // truncation number of bits
