...
bool ASSegPropagation::runOnModule(Module &M) {
  ...
  // Iterates over every memory node |M| and over every send node |S|. Calls
  // dfsVisit O(|M| * |S|) times
  errs() << "Iterates over every memory node |M| and over every send node |S| "
            "... ";
  // Loop for P1

  for (std::set<GraphNode *>::iterator send1it = sendP1.begin(),
                                       send1end = sendP1.end();
       send1it != send1end; ++send1it) {
    for (std::set<GraphNode *>::iterator mem1it = mem1.begin(),
                                         mem1end = mem1.end();
         mem1it != mem1end; ++mem1it) {
    ...
    if (tablesOfSend.size() > 0) {
      ...
      if (tablesOfSend.size() > 1) {
        std::set<SegmentationTable *>::iterator t1 = tablesOfSend.begin(),
                                                tend = tablesOfSend.end();
  ...
  // Loop for P2
  for (std::set<GraphNode *>::iterator send2it = sendP2.begin(),
                                       send2end = sendP2.end();
       send2it != send2end; ++send2it) {
    for (std::set<GraphNode *>::iterator mem2it = mem2.begin(),
                                         mem2end = mem2.end();
         mem2it != mem2end; ++mem2it) {
      // Get the reachable memnodes' tables
    ...
    if (tablesOfSend.size() > 0) {
      ...
      if (tablesOfSend.size() > 1) {
        std::set<SegmentationTable *>::iterator t2 = tablesOfSend.begin(),
                                                tend = tablesOfSend.end();
  ...
  // Iterates over every Send node |S| and over every Recv node |R|. Calls
  // dfsVisit O(|S| * |R|) times

  // Loop P1
  for (std::set<GraphNode *>::iterator recv1it = recvP1.begin(),
                                       recv1end = recvP1.end();
       recv1it != recv1end; ++recv1it) {
    for (std::set<GraphNode *>::iterator send2it = sendP2.begin(),
                                         send2end = sendP2.end();
         send2it != send2end; ++send2it) {
    ...
    if (allSendsToRecvTables1.size() > 0) {
      ...
      if (allSendsToRecvTables1.size() > 1) {
        std::set<SegmentationTable *>::iterator t1 = allSendsToRecvTables1
                                                         .begin(),
                                                tend1 =
  ...
  // Loop P2
  for (std::set<GraphNode *>::iterator recv2it = recvP2.begin(),
                                       recv2end = recvP2.end();
       recv2it != recv2end; ++recv2it) {
    for (std::set<GraphNode *>::iterator send1it = sendP1.begin(),
                                         send1end = sendP1.end();
         send1it != send1end; ++send1it) {
    ...
    if (allSendsToRecvTables2.size() > 0) {
      ...
      if (allSendsToRecvTables2.size() > 1) {
        std::set<SegmentationTable *>::iterator t2 = allSendsToRecvTables2
                                                         .begin(),
                                                tend2 =
    ...
    for (std::set<GraphNode *>::iterator mem1it = mem1.begin(),
      ...
      if ((*recv1it)->getSuccessors().count(*mem1it)) {
        if (dg->dfsVisit(*recv1it, *mem1it, visited) &&
          ...
          SegmentationTable *recvMemSegTab =
              allSendsToRecvTableMap1[(*recv1it)];

          if (destMemSegTab != NULL && recvMemSegTab != NULL) {
            SegmentationTable *auxTab =
                ass.UnionMerge(destMemSegTab, recvMemSegTab);
    ...
    for (std::set<GraphNode *>::iterator mem2it = mem2.begin(),
      ...
      if ((*recv2it)->getSuccessors().count(*mem2it)) {
        if (dg->dfsVisit(*recv2it, *mem2it, visited) &&
          ...
          SegmentationTable *destMemSegTab = ass.getTable(key);
          SegmentationTable *recvMemSegTab =
              allSendsToRecvTableMap1[(*recv2it)];
  ...
  for (std::map<int, SegmentationTable *>::iterator it = memToTable.begin(),
                                                    itend = memToTable.end();
       it != itend; ++it) {
    NumPropSegments += it->second->row_num;

    for (set<SegmentationTableRow *>::iterator r = (*it).second->rows.begin(),
                                               e = (*it).second->rows.end();
         r != e; ++r) {
...
Range ASSegPropagation::getRange(Value *Pointer) {
  ...
  for (std::set<SegmentationTableRow *>::iterator it = seg->rows.begin(),
                                                  itend = seg->rows.end();
       it != itend; ++it) {
    for (std::set<Value *>::iterator itval = (*it)->pointers.begin(),
                                     itvalend = (*it)->pointers.end();
         itval != itvalend; ++itval) {
      if ((*itval)->getName() == Pointer->getName()) {
        errs() << "Pointer here. Row Range: ";
        (*it)->content.print(errs());
        errs() << "\n";
...
bool ASSegPropagation::isSend(GraphNode *node) {
  if ((node->getLabel().find("Write") != std::string::npos)) return true;

  return false;
}

bool ASSegPropagation::isRecv(GraphNode *node) {
  if ((node->getLabel().find("Read") != std::string::npos)) return true;

  return false;
}

bool ASSegPropagation::isNodeP1(GraphNode *node) {
  if (node->getName().find("P1") != std::string::npos) return true;

  return false;
}

char ASSegPropagation::ID = 0;
static RegisterPass<ASSegPropagation> X(
    "as-set-propagation", "Propagation of segmentation of Alias Sets", false,
    false); /* namespace llvm */
