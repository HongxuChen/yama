...
bool compAPInt(APInt i, APInt j) { return (i.slt(j)); }
...
APInt  // returns
    ...
    (APInt x, APInt y)  // parameters
{
  if (x == Zero or y == Zero) return Zero;
...
int SegmentationTables::getSumBehind(std::vector<int> v, int i) {
  int s = 0;
  if (i > v.size()) i = v.size();
  for (int j = i - 1; j >= 0; j--) s += v[j];
  return s;
}

// Returns the type of the ith element inside type
Type* SegmentationTables::getTypeInside(Type* type, int i) {
  if (type->isPointerTy())
    ...
    (Type* type)  // Parameter
{
  ...
  for (int i = 0; i < NumPrimitives.size(); i++)
    if (NumPrimitives[i]->type == type) return NumPrimitives[i]->num;
    ...
    (Type* type)  // Parameter
{
  ...
  for (int i = 0; i < PrimitiveLayouts.size(); i++)
    if (PrimitiveLayouts[i]->type == type) return PrimitiveLayouts[i]->layout;
  ...
  if (type->isArrayTy()) {
    ...
    Type* arrtype = type->getArrayElementType();
    int arrtypenum = getNumPrimitives(arrtype);
    for (int i = 0; i < num; i++) pm[i] = arrtypenum;
    ...
    ()  // Parameters
{
  ...
  for (llvm::DenseMap<Value*, RangedPointer*>::iterator
           i = RangedPointerMap.begin(),
           e = RangedPointerMap.end();
       i != e; i++) {
    errs() << "[";
    ...
    errs() << "] -> " << *(i->first) << " fathered by " << *(i->second->father)
           << "\n";
  }
  errs() << "-------------------------\n\n";
}

void SegmentationTables::printNotUsedPointers() {
  errs() << "\n-------------------------\nNot Used Pointers:"
         << "\n";

  for (std::set<Value*>::iterator i = NotUsedPointers.begin(),
                                  e = NotUsedPointers.end();
       i != e; ++i) {
    errs() << **i << "\n";
  }
...
void SegmentationTables::printSegmentationTableMap() {
  errs() << "\n-------------------------\nRanged Alias Tables:"
         << "\n";

  for (llvm::DenseMap<Value*, SegmentationTable*>::iterator
           i = SegmentationTableMap.begin(),
           e = SegmentationTableMap.end();
       i != e; i++) {
    errs() << "Table: " << *(i->first) << "\n";
    for (set<SegmentationTableRow*>::iterator ii = i->second->rows.begin(),
                                              ee = i->second->rows.end();
         ii != ee; ii++) {
      errs() << "Row: [";
...
void SegmentationTable::PrintPointers() {
  errs() << "\nTable: " << *(base) << "\n";
  for (set<SegmentationTableRow*>::iterator ii = rows.begin(), ee = rows.end();
       ii != ee; ii++) {
    errs() << "Row: [";
    if ((*ii)->offset.getLower() == Min)
      errs() << "-inf, ";
...
void SegmentationTable::PrintContent() {
  errs() << "\nTable: " << *(base) << "\n";
  for (set<SegmentationTableRow*>::iterator ii = rows.begin(), ee = rows.end();
       ii != ee; ii++) {
    errs() << "Row: [";
...
std::set<Value*> SegmentationTables::getNotUsedPointers() {
...
static RegisterPass<SegmentationTables> X(
    "segmentation-tables",
    "Get tables memory ranges and pointers that can point for such ranges",
    false, false);
...
{
  /*
  * Gets RangeAnalysis Pass Data, declares some auxiliary sets and initiates
  * statistics.
  */
    ...
    */
    for (inst_iterator I = inst_begin(F), E = inst_end(F); I != E; ++I) {
      ...
      if (isa<AllocaInst>(*I)) {
        ...
        DenseMap<Value*, RangedPointer*>::iterator base_it =
            RangedPointerMap.find(base_ptr);
        if (base_it == RangedPointerMap.end()) {
          NotUsedPointers.insert(i);
          continue;
        }
        RangedPointer* base_range = base_it->second;
        RangedPointerMap[i] = new RangedPointer(
            i, base_range->offset.getLower(), base_range->offset.getUpper(),
            base_range->father, F);
      ...
      } else if (isa<GetElementPtrInst>(*I))  // GEP processing
      {
        ...
        int base_ptr_num_primitive =
            getNumPrimitives(base_ptr_type->getPointerElementType());
        ...
        if (isa<ConstantInt>(*indx)) {
          ...
          int higher_bitwidth =
              (lower_range.getBitWidth() > constant.getBitWidth())
          ...
          lower_range =
              safe_addition(lower_range, safe_multiplication(bpnp, constant));
          higher_range =
              safe_addition(higher_range, safe_multiplication(bpnp, constant));
          ...
          if (!r.isUnknown()) {
            ...
            lower_range =
                safe_addition(lower_range, safe_multiplication(bpnp, rl));
            higher_range =
                safe_addition(higher_range, safe_multiplication(bpnp, ru));
          }
        }
        ...
        for (User::op_iterator idxe = ((GetElementPtrInst*)i)->idx_end();
             idx != idxe; idx++) {
          ...
          std::vector<int> base_ptr_primitive_layout =
              getPrimitiveLayout(base_ptr_type);
          ...
          if (isa<ConstantInt>(*indx)) {
            ...
            lower_range = safe_addition(
                lower_range,
                APInt(higher_bitwidth, getSumBehind(base_ptr_primitive_layout,
                                                    constant.getSExtValue())));
            higher_range = safe_addition(
                higher_range,
                APInt(higher_bitwidth, getSumBehind(base_ptr_primitive_layout,
                                                    constant.getSExtValue())));
            ...
            if (!r.isUnknown()) {
              ...
              int higher_bitwidth =
                  (lower_range.getBitWidth() > rl.getBitWidth())
                      ? lower_range.getBitWidth()
                      : rl.getBitWidth();
              higher_bitwidth = (higher_range.getBitWidth() > higher_bitwidth)
                                    ? higher_range.getBitWidth()
        ...
        RangedPointerMap[i] = new RangedPointer(i, lower_range, higher_range,
                                                base_range->father, F);

      }  // end of GEP processing
      else {
        Type* non_type = i->getType();
        if (non_type->isPointerTy()) NotUsedPointers.insert(i);
      }
  ...
  */
  ...
  for (std::set<Value*>::iterator i = AlocSet.begin(), e = AlocSet.end();
       i != e; i++) {
    for (llvm::DenseMap<Value*, RangedPointer*>::iterator
             ii = RangedPointerMap.begin(),
             ee = RangedPointerMap.end();
         ii != ee; ii++) {
      if (*i == ii->second->father) RangedPointerSets[*i].insert(ii->second);
    }
  }
  ...
  */
  ...
  for (llvm::DenseMap<Value*, std::set<RangedPointer*> >::iterator
           i = RangedPointerSets.begin(),
           e = RangedPointerSets.end();
       i != e; ++i) {
    APInt MinusOne = Zero;
    MinusOne--;
    APInt One = Zero;
    ...
    while (true) {
      ...
      higher_range = new_higher_range1.slt(new_higher_range2)
                         ? new_higher_range1
                         : new_higher_range2;
      ...
      for (std::set<RangedPointer*>::iterator ii = i->second.begin(),
        ...
        if ((*ii)->offset.getLower().sle(higher_range) and
          ...
          SegmentationTableMap[i->first]
              ->pointer_to_rows[(*ii)->pointer]
              .insert(table_row);
          mrk = true;
        }
      }
  ...
  */
  ...
  for (DenseMap<Value*, SegmentationTable*>::iterator
           i = SegmentationTableMap.begin(),
           e = SegmentationTableMap.end();
       i != e; i++) {
    ...
    if (isa<AllocaInst>(*(i->first))) {
      ...
      if (cf)
        if (strcmp(cf->getName().data(), "malloc") == 0 or
            strcmp(cf->getName().data(), "calloc") == 0) {
          i->second->aloc_base = true;
        }
    } else {
    ...
    if (tsize > 1) {
      NTables++;
      NTableAvgSize += tsize;
      if (tsize > NTableMaxSize) NTableMaxSize = tsize;
    }
  ...
  if (NTables > 0) NTableAvgSize /= NTables;
