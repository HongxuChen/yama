...
STATISTIC(NDefined,
          "Number of relevant segments with well defined content range");
...
void AliasSetSegmentation::ContentMerge(SegmentationTable* result,
                                        SegmentationTable* one,
                                        SegmentationTable* two) {
  // for each row of result
  for (std::set<SegmentationTableRow*>::iterator i = result->rows.begin(),
                                                 e = result->rows.end();
       i != e; i++) {
    ...
    // for each row of one
    for (std::set<SegmentationTableRow*>::iterator ii = one->rows.begin(),
                                                   ee = one->rows.end();
         ii != ee; ii++) {
    ...
    // for each row of two
    for (std::set<SegmentationTableRow*>::iterator ii = two->rows.begin(),
                                                   ee = two->rows.end();
         ii != ee; ii++) {
...
bool AliasSetSegmentation::CommonPointers(SegmentationTable* one,
                                          SegmentationTable* two) {
  for (std::set<RangedPointer*>::iterator i = one->ranged_pointers.begin(),
                                          e = one->ranged_pointers.end();
       i != e; i++)
    for (std::set<RangedPointer*>::iterator ii = two->ranged_pointers.begin(),
                                            ee = two->ranged_pointers.end();
         ii != ee; ii++)
      if ((*i)->pointer == (*ii)->pointer) return true;
  return false;
}
...
void AliasSetSegmentation::BuildTable(SegmentationTable* result) {
  ...
  while (true) {
    ...
    for (
        std::set<RangedPointer*>::iterator ii = result->ranged_pointers.begin(),
                                           ee = result->ranged_pointers.end();
        ii != ee; ++ii) {
      if ((*ii)->offset.getUpper().slt(new_higher_range1) and
          (*ii)->offset.getUpper().sgt(higher_range))
        new_higher_range1 = (*ii)->offset.getUpper();

      if ((*ii)->offset.getLower().slt(new_higher_range2) and
          (*ii)->offset.getLower().sgt(new_lower_range))
        new_higher_range2 = (*ii)->offset.getLower() - One;
    }

    // refreshing ranges
    lower_range = new_lower_range;
    higher_range = new_higher_range1.slt(new_higher_range2) ? new_higher_range1
                                                            : new_higher_range2;
    ...
    for (
        std::set<RangedPointer*>::iterator ii = result->ranged_pointers.begin(),
                                           ee = result->ranged_pointers.end();
        ii != ee; ++ii) {
      if ((*ii)->offset.getLower().sle(higher_range) and
          (*ii)->offset.getUpper().sge(lower_range)) {
    ...
    if (higher_range == Max) break;
  }
}

SegmentationTable* AliasSetSegmentation::OneLineTransformation(
  ...
  for (std::set<RangedPointer*>::iterator i = Table->ranged_pointers.begin(),
                                          e = Table->ranged_pointers.end();
       i != e; i++)
    result->ranged_pointers.insert(
        new RangedPointer((*i)->pointer, Min, Max, (*i)->father, (*i)->func));
...
SegmentationTable* AliasSetSegmentation::UnionMerge(SegmentationTable* one,
                                                    SegmentationTable* two) {
  SegmentationTable* result = new SegmentationTable();
  // go through tables ranged pointers adding them to the result
  for (std::set<RangedPointer*>::iterator i = one->ranged_pointers.begin(),
  ...
  for (std::set<RangedPointer*>::iterator i = two->ranged_pointers.begin(),
                                          e = two->ranged_pointers.end();
       i != e; i++)
    result->ranged_pointers.insert(*i);
  // building result table
  BuildTable(result);
  ...
  if (one->AliasSet == two->AliasSet) result->AliasSet = one->AliasSet;

  return result;
}

SegmentationTable* AliasSetSegmentation::OffsetMerge(SegmentationTable* one,
                                                     SegmentationTable* two,
                                                     Range Offset) {
  SegmentationTable* result = new SegmentationTable();
  // go through tables ranged pointers adding them to the result
  ...
  for (std::set<RangedPointer*>::iterator i = two->ranged_pointers.begin(),
    ...
    result->ranged_pointers.insert(
        new RangedPointer((*i)->pointer, new_offset.getLower(),
                          new_offset.getUpper(), (*i)->father, (*i)->func));
  }
  // building result table
  BuildTable(result);
...
SegmentationTable* AliasSetSegmentation::MultiplicationMerge(
...
bool AliasSetSegmentation::runOnModule(Module& M) {
  ...
  /** ///testing alias sets
  for (llvm::DenseMap<int, std::set<Value*> >::iterator i = vs.begin(), e =
  vs.end(); i != e; ++i)
  {
                  errs() << "Set " << i->first << " :\n";
                  for (std::set<Value*>::iterator ii = i->second.begin(), ee =
  ...
  //
  for (DenseMap<Value*, SegmentationTable*>::iterator i = stm.begin(),
                                                      e = stm.end();
       i != e; i++) {
  ...
  //
  // for each alias sets
  for (llvm::DenseMap<int, std::set<Value*> >::iterator i = vs.begin(),
                                                        e = vs.end();
       i != e; i++) {
    for (std::set<SegmentationTable*>::iterator
             ii = tables_sets[i->first].begin(),
             ee = tables_sets[i->first].end();
         ii != ee; ii++) {
      ...
      if (table->aloc_base) {
        ...
        for (llvm::Value::use_iterator ui = f->use_begin(), ue = f->use_end();
             ui != ue; ui++) {
          User* u = *ui;
          if (isa<CallInst>(*u)) {
        ...
        for (std::set<RangedPointer*>::iterator
                 pi = table->ranged_pointers.begin(),
                 pe = table->ranged_pointers.end();
             pi != pe; pi++)
  ...
  /** ///testing connections
  errs() << "CONNECTIONS\n-------------------\n\n";
  for(llvm::DenseMap<Value*, std::set<Value*> >::iterator i =
  connections.begin(), e = connections.end(); i != e; i++)
  {
          errs() << *(i->first) << "\n  Connects with:\n";
          for(std::set<Value*>::iterator ii = i->second.begin(), ee =
  i->second.end(); ii != ee; ii++)
  ...
  errs() << "\nLOOSE POINTERS\n-------------------\n";
  for(llvm::DenseMap<int, std::set<Value*> >::iterator i =
  loose_pointers.begin(), e = loose_pointers.end(); i != e; i++)
  {
  ...
  for (llvm::DenseMap<int, std::set<Value*> >::iterator i = vs.begin(),
    ...
    //"\n--------------\n";
    //																														for(std::set<SegmentationTable*>::iterator
    //ti = tables_to_merge.begin(), te = tables_to_merge.end(); ti != te; ti++)
    //																														{
    //																															SegmentationTable*
    //table = *ti;
    //																															//table->PrintPointers();
    ...
    while (true) {
      ...
      for (std::set<SegmentationTable*>::iterator ti = tables_to_merge.begin(),
                                                  te = tables_to_merge.end();
           ti != te; ti++) {
        SegmentationTable* table = *ti;
        Value* base = table->base;
        // if a table has no connections and is not an allocation table, then
        // add its pointers to loose and remove it from the set
        if (connections[base].empty()) {
          ...
          for (std::set<RangedPointer*>::iterator
                   ...
                   pe = table->ranged_pointers.end();
               pi != pe; pi++) {
            if (loose_pointers[i->first].find((*pi)->pointer) ==
                loose_pointers[i->first].end())
              loose_pointers[i->first].insert((*pi)->pointer);
          }
      ...
      for (std::set<SegmentationTable*>::iterator ti = tables_to_remove.begin(),
                                                  te = tables_to_remove.end();
           ti != te; ti++) {
      ...
      // for each connection
      for (std::set<Value*>::iterator ci = connections[table2->base].begin(),
                                      ce = connections[table2->base].end();
           ci != ce; ci++) {
        Value* connected = *ci;
        ...
        for (std::set<SegmentationTable*>::iterator
                 tii = tables_to_merge.begin(),
                 tee = tables_to_merge.end();
             tii != tee; tii++) {
          SegmentationTable* table = *tii;
          RangedPointer* rp =
              RangedPointer::FindByValue(connected, table->ranged_pointers);
          if (rp == NULL) continue;  // is not in this table
          // is in the table
          tables_connected.insert(table);
          loose_connection = false;
        }
        // if a connection goes to a pointer not in a table, put pointers of
        // that table in loose pointers, and go to the next connection |don't
        // stop
        if (loose_connection) {
          for (std::set<RangedPointer*>::iterator
                   pi = table2->ranged_pointers.begin(),
                   pe = table2->ranged_pointers.end();
               pi != pe; pi++) {
            if (loose_pointers[i->first].find((*pi)->pointer) ==
                loose_pointers[i->first].end())
              loose_pointers[i->first].insert((*pi)->pointer);
        ...
        for (std::set<SegmentationTable*>::iterator
                 tii = tables_connected.begin(),
                 tee = tables_connected.end();
             tii != tee; tii++) {
          ...
          // if they are diferent
            ...
            // errs() << "DIFF\n";
            // if they have common pointers, MultiplicationMerge into
            // tables_to_add and add table1 to tables_to_remove |don't stop
            ...
            // if they don't have common pointers, OffsetMerge into
            // tables_to_add and add table1 to tables_to_remove |don't stop
            else {
              // errs() << "OFFSET\n";
              RangedPointer* rp = RangedPointer::FindByValue(
                  connected, table1->ranged_pointers);
      ...
      for (std::set<SegmentationTable*>::iterator ti = tables_to_add.begin(),
                                                  te = tables_to_add.end();
           ti != te; ti++) {
      ...
      //convergence
      //																														for(std::set<SegmentationTable*>::iterator
      //ti = tables_to_merge.begin(), te = tables_to_merge.end(); ti != te;
      //ti++)
      //																														{
      //																															SegmentationTable*
      //table = *ti;
  ...
  for (llvm::DenseMap<int, std::set<Value*> >::iterator i = vs.begin(),
    ...
    for (std::set<Value*>::iterator ii = i->second.begin(),
      ...
      if ((master_tables[i->first] == NULL)) {
        ...
        if (loose_pointers[i->first].find(v) ==
            loose_pointers[i->first].end()) {
          // add value to loose_pointers
          loose_pointers[i->first].insert(v);
        }
      } else {
        // if it is not present in the master table and also not already present
        // in loose_pointers
        if ((RangedPointer::FindByValue(
                 v, master_tables[i->first]->ranged_pointers) == NULL) &&
            (loose_pointers[i->first].find(v) ==
             loose_pointers[i->first].end())) {
          // add value to loose_pointers
          loose_pointers[i->first].insert(v);
        } /**/
  ...
  for (llvm::DenseMap<int, std::set<Value*> >::iterator i = vs.begin(),
    ...
    if (master_tables[i->first] == NULL) {
      ...
      for (std::set<Value*>::iterator ii = loose_pointers[i->first].begin(),
                                      ee = loose_pointers[i->first].end();
           ii != ee; ii++)
        master_tables[i->first]->ranged_pointers.insert(
            new RangedPointer((*ii), Min, Max, NULL, NULL));
      ...
      for (std::set<Value*>::iterator ii = loose_pointers[i->first].begin(),
        ...
        for (set<SegmentationTableRow*>::iterator
                 iii = master_tables[i->first]->rows.begin(),
                 eee = master_tables[i->first]->rows.end();
             iii != eee; iii++) {
          // add it to row
  ...
  errs()
      << "END OF MASTER TABLES CALCULATION AND START OF CONTENT CALCULATION\n";
  //
  ...
  for (llvm::DenseMap<int, std::set<Value*> >::iterator i = vs.begin(),
    ...
    for (std::set<SegmentationTableRow*>::iterator
             ii = master_tables[i->first]->rows.begin(),
             ee = master_tables[i->first]->rows.end();
         ii != ee; ii++) {
    ...
    for (llvm::DenseMap<Value*, std::set<SegmentationTableRow*> >::iterator
             ii = master_tables[i->first]->pointer_to_rows.begin(),
             ee = master_tables[i->first]->pointer_to_rows.end();
         ii != ee; ii++) {
      Value* pointer = ii->first;
      // for each of its Users
      for (llvm::Value::use_iterator ui = pointer->use_begin(),
                                     ue = pointer->use_end();
           ui != ue; ui++) {
        User* u = *ui;
        ...
        if (isa<StoreInst>(*u))
          if (((StoreInst*)u)->getPointerOperand() == pointer) {
            ...
            if (isa<ConstantInt>(*stored)) {
              to_content = Range(((ConstantInt*)stored)->getValue(),
                                 ((ConstantInt*)stored)->getValue());
            } else {
  ...
  for (llvm::DenseMap<int, SegmentationTable*>::iterator
           i = master_tables.begin(),
           e = master_tables.end();
       i != e; i++) {
    int nms = 0;
    SegmentationTable* table = i->second;
    nt++;
    for (std::set<SegmentationTableRow*>::iterator ri = table->rows.begin(),
                                                   re = table->rows.end();
         ri != re; ri++) {
      ...
      if (!r->pointers.empty()) {
        NSegments++;
        nms++;
        if ((r->content.getLower().sgt(Min)) or
            (r->content.getUpper().slt(Max)))
          NDefined++;
      }
    }
    if (nms > NMaxSegments) NMaxSegments = nms;
  }
  if (nt > 0) NAvgSegments = NSegments / nt;
...
static RegisterPass<AliasSetSegmentation> X("as-segmentation",
                                            "Segmentation of Alias Sets", false,
                                            false);
/* namespace llvm */
...
/*//testing merges
        ...
        int c = 0;
        for(DenseMap<Value*, SegmentationTable*>::iterator i = stm.begin(), e =
   stm.end(); i != e; i++)
        {
                t[c] = i->second;
                c++;
