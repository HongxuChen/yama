//===---------------------------- vSSA.cpp --------------------------------===//
//===Converts programs into partial Extended Static Single Assignment form-===//
//
// Copyright (C) 2011, 2015    Victor Hugo Sperle Campos
//
...
void vSSA::insertSigmas(TerminatorInst *TI, Value *V) {
    ...
    for (unsigned i = 0, e = TI->getNumSuccessors(); i < e; ++i) {
        ...
        if (BB_next != BB && BB_next->getSinglePredecessor() != NULL &&
            ...
            if (verifySigmaExistance(V, BB_next, BB)) continue;
        ...
        // inserted in them
        if (firstSigma && !vssaphi_created.empty()) firstSigma = false;
    }
...
void vSSA::renameUsesToPhi(Value *V, PHINode *phi) {
    ...
    for (unsigned k = 0, f = phi->getNumIncomingValues(); k < f; ++k) {
        ...
        if (!p || !p->getName().startswith(vSSA_SIG)) continue;
...
bool vSSA::dominateAny(BasicBlock *BB, Value *value) {
    ...
    if (Instruction *I = dyn_cast<Instruction>(value)) {
        if (BB == I->getParent()) return false;
    } else if (Argument *A = dyn_cast<Argument>(value)) {
        if (BB == &A->getParent()->getEntryBlock()) return false;
    }
...
bool vSSA::dominateOrHasInFrontier(BasicBlock *BB, BasicBlock *BB_next,
    ...
    for (Value::user_iterator begin = value->user_begin(),
        ...
        BasicBlock *BB_father = I->getParent();
        if (BB_next == BB_father && isa<PHINode>(I)) continue;

        if (DT_->dominates(BB_next, BB_father)) return true;
