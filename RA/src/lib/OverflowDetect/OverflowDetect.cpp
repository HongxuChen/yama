...
static cl::opt<bool, false> UseRaPrunning(
    "use-ra-prunning",
    cl::desc("Use range analysis to avoid inserting unnecessary checks."),
    cl::NotHidden);

static cl::opt<bool, false> InsertAborts(
    "insert-ovf-aborts",
    cl::desc("Abort the instrumented program when an overflow occurs."),
    cl::NotHidden);

static cl::opt<bool, false> InsertFprintfs(
    "insert-stderr-fprintfs",
    cl::desc("Insert fprintf calls when overflow occurs."), cl::NotHidden,
    cl::init(true));
...
struct OverflowDetect : public ModulePass {
  ...
  bool isLimited(const Range& range) {
    return range.isRegular() && range.getLower().ne(Min) &&
           range.getUpper().ne(Max);
  }

  OvfPrediction ovfStaticAnalysis(Instruction* I, InterProceduralRA* ra);

  virtual void getAnalysisUsage(AnalysisUsage& AU) const {
    if (UseRaPrunning) AU.addRequired<InterProceduralRA>();
  }
  ...
  Value* GVstderr, *FPrintF, *overflowMessagePtr, *truncErrorMessagePtr,
      *overflowMessagePtr2, *truncErrorMessagePtr2;
...
STATISTIC(NrPossibleOvfStaticallyDetected,
          "Number of possible overflows statically detected");

static RegisterPass<OverflowDetect> X("overflow-detect",
                                      "OverflowDetect Instrumentation Pass");
...
OvfPrediction OverflowDetect::ovfStaticAnalysis(Instruction* I,
  ...
  if (!isLimited(r)) {
    ...
    // Check if the range is contained within the type bounds, because it is
    // represented with less bits
    if (r.getUpper().getBitWidth() < numBits ||
        r.getLower().getBitWidth() < numBits) {
      NrPrunedInsts++;
    ...
    if (isSignedInst(I)) {
      ...
      // So, I'm using the signed max value to do the analysis. It needs to be
      // fixed.
      APInt MaxValue = APInt::getSignedMaxValue(numBits);
 ...
 * It receives an argument that tells what is the next basic block to be
 * executed.
 */
BasicBlock* OverflowDetect::NewOverflowOccurrenceBlock(Instruction* I,
...
void OverflowDetect::InsertGlobalDeclarations() {
  ...
  FPrintF = module->getOrInsertFunction(
      "fprintf", FunctionType::get(Type::getVoidTy(*context), Params, true));
  ...
  if (InsertAborts) {
    ...
    // Before declaring the abort function, we need to check whether it is
    // already declared or not
...
/*
 * 	In order to be valid, an instruction must be of intBITWIDTH type, and
...
bool OverflowDetect::runOnModule(Module& M) {
  ...
  for (Module::iterator Fit = M.begin(), Fend = M.end(); Fit != Fend; ++Fit) {
    ...
    if (InsertAborts) {
      // Create the basic block which the controw flow goes to when the
      // assertion fail
      AbortBB =
          BasicBlock::Create(*context, "assert fail", dyn_cast<Function>(Fit));
    ...
    for (Function::iterator BBit = Fit->begin(), BBend = Fit->end();
      ...
      for (BasicBlock::iterator Iit = BBit->begin(); Iit != BBit->end();
           ++Iit) {
        ...
        if (isValidInst(I) && !IsNotOriginal(*I)) {
          ...
          if (Pred != OvWillNotHappen) insertInstrumentation(I, AbortBB, Pred);
        }
      }
    }
  }
...
void OverflowDetect::MarkAsNotOriginal(Instruction& inst) {
  inst.setMetadata("new-inst",
                   MDNode::get(*context, llvm::ArrayRef<Metadata*>()));
}
