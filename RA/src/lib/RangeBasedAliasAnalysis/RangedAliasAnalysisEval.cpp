...
namespace llvm {
    ...
    class RAAEval : public FunctionPass {
        unsigned NoAlias1a, MayAlias1a, PartialAlias1a, MustAlias1a, NTNoAlias,
                Total1, Total2;
    ...
    static RegisterPass<RAAEval> X(
            "raa-eval", "Exhaustive Ranged Alias Analysis Precision Evaluator", false,
            false);
        ...
        */
        ...
        for (llvm::DenseMap<Value *, RangedAliasTable *>::iterator
            ...
            if (i->second->function == &F) {
                ...
                int ind = 0;
                for (std::set<RangedPointer *>::iterator ii = MRS.begin(), ee = MRS.end();
                     ii != ee; ii++) {
                    MRV[ind] = (*ii);
                    ind++;
                }
                ...
                for (int j = 0; j < ind; j++)
                    for (int k = j + 1; k < ind; k++) {
                        ...
                        Type *I1ElTy =
                                cast<PointerType>((MRV[j]->pointer)->getType())->getElementType();
                        // if (I1ElTy->isSized()) I1Size = AA.getTypeStoreSize(I1ElTy);
                        uint64_t I2Size = AliasAnalysis::UnknownSize;
                        Type *I2ElTy =
                                cast<PointerType>((MRV[k]->pointer)->getType())->getElementType();
                        // if (I2ElTy->isSized()) I2Size = AA.getTypeStoreSize(I2ElTy);

                        AliasAnalysis::AliasResult ranged =
                                alias(MRV[j]->pointer, MRV[k]->pointer);
                        AliasAnalysis::AliasResult other =
                                AA.alias(MRV[j]->pointer, I1Size, MRV[k]->pointer, I2Size);
                        Total1++;
                        switch (other) {
                        ...
                        if (ranged == AliasAnalysis::NoAlias ||
                            other == AliasAnalysis::NoAlias) {
                            NoAlias1b++;
                            if (i->second->is_array == true)
                                ArrayNAb++;
                            else if (i->second->is_struct == true)
        ...
        for (inst_iterator I = inst_begin(F), E = inst_end(F); I != E; ++I) {
            ...
            if (CallSite CS = cast<Value>(&Inst)) {
                ...
                for (CallSite::arg_iterator AI = CS.arg_begin(), AE = CS.arg_end();
                     AI != AE; ++AI)
                    if (isInterestingPointer(*AI)) Pointers.insert(*AI);
                CallSites.insert(CS);
            } else {
...
*
*/
    AliasAnalysis::AliasResult RAAEval::alias(Value *p1, Value *p2) {
        RangedPointer *m1 = RangedPointerMap[p1];
        RangedPointer *m2 = RangedPointerMap[p2];
        RangedAliasTable *t1, *t2;
        if (m1 != NULL)
            t1 = RangedAliasTableMap[m1->father];
        else
            t1 = NULL;
        if (m2 != NULL)
            t2 = RangedAliasTableMap[m2->father];
        else
            t2 = NULL;

        if ((t1 == t2) && (t1 != NULL)) {
            bool mark = false;
            if (m1->offset.getLower().slt(m2->offset.getLower()) &&
                m1->offset.getUpper().slt(m2->offset.getLower()))
                mark = true;
            else if (m2->offset.getLower().slt(m1->offset.getLower()) &&
                     m2->offset.getUpper().slt(m1->offset.getLower()))
                mark = true;
