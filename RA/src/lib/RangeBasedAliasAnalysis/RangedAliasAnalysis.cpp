...
static RegisterPass<RangedAliasAnalysis> X(
    "ranged-aa", "RangedAliasTables-based Alias Analysis", false, false);
static RegisterAnalysisGroup<AliasAnalysis> E(X);

void RangedAliasAnalysis::getAnalysisUsage(AnalysisUsage &AU) const {
  AliasAnalysis::getAnalysisUsage(AU);
  AU.addRequired<RangedAliasTables>();
  AU.setPreservesAll();
}

bool RangedAliasAnalysis::runOnFunction(Function &F) {
...
RangedAliasAnalysis::AliasResult RangedAliasAnalysis::alias(
...
RangedAliasTable *RangedAliasAnalysis::getTable(Value *p) {
  for (llvm::DenseMap<Value *, RangedAliasTable *>::iterator
    ...
    for (set<RangedAliasTableRow *>::iterator ii = table->rows.begin(),
                                              ee = table->rows.end();
         ii != ee; ii++) {
      if ((*ii)->pointers.find(p) != (*ii)->pointers.end()) return table;
    }
  }
  return NULL;
}
