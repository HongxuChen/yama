...
int RangedAliasTables::getSumBehind(std::vector<int> v, int i) {
    int s = 0;
...
Type *RangedAliasTables::getTypeInside(Type *type, int i) {
    if (type->isPointerTy())
...
void RangedAliasTables::printNotUsedPointers() {
    errs() << "\n-------------------------\nNot Used Pointers:"
    << "\n";
...
void RangedAliasTables::printRangedAliasTableMap() {
    errs() << "\n-------------------------\nRanged Alias Tables:"
    << "\n";

    for (llvm::DenseMap<Value *, RangedAliasTable *>::iterator
                 i = RangedAliasTableMap.begin(),
                 e = RangedAliasTableMap.end();
         i != e; i++) {
        errs() << "Table: " << *(i->first) << "\n";
...
std::set<Value *> RangedAliasTables::getNotUsedPointers() {
...
static RegisterPass<RangedAliasTables> X(
        "ranged-alias-tables",
        ...
        (Module &M)  // Parameters
{
        ...
        while (true) {
            ...
            for (std::set<RangedPointer *>::iterator ii = i->second.begin(),
                ...
                if ((*ii)->offset.getLower().sle(higher_range) and
                    ...
                    RangedAliasTableMap[i->first]->pointer_to_rows[(*ii)->pointer].insert(
                            table_row);
                    mrk = true;
                }
            }
    ...
    */
    ...
    for (DenseMap<Value *, RangedAliasTable *>::iterator
                 i = RangedAliasTableMap.begin(),
                 e = RangedAliasTableMap.end();
         i != e; i++) {
        ...
        if (isa<AllocaInst>(*(i->first))) {
            ...
            if (cf) if (strcmp(cf->getName().data(), "malloc") == 0 or
