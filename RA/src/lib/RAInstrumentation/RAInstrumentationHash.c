...
void initHash() { memset(hash, 0, sizeof(hash)); }
...
void setCurrentMinMax(void *ptr, int Value) {
    if (Value < getCurrentMin(ptr)) setCurrentMin(ptr, Value);

    if (Value > getCurrentMax(ptr)) setCurrentMax(ptr, Value);
}

char *getFileName(const char *prefix, char *moduleName, const char *suffix) {
    char *result = (char *) calloc(
            strlen(prefix) + strlen(moduleName) + strlen(suffix), sizeof(char));
    result[0] = '\0';
...
void printHash(char *moduleName) {
    ...
    for (i = 0; i < RA_HASH_SIZE; i++) {
        if (hash[i].ptr != 0) {
            fprintf(file, "%d %d %d\n", (int) hash[i].ptr, hash[i].minValue,
                    hash[i].maxValue);
            list = hash[i].list;

            while (list != 0) {
                fprintf(file, "%d %d %d\n", (int) list->ptr, list->minValue,
                        list->maxValue);
                list = list->next;
            }
        }
