...
STATISTIC(raInstrumentationNumInstructions,
          "Number of instructions instrumented");
...
void RAInstrumentation::InstrumentMainFunction(Function* F,
  ...
  GlobalVariable* moduleName = new GlobalVariable(
      *module, stringConstant->getType(), true,
      llvm::GlobalValue::InternalLinkage, stringConstant, "moduleName");

  Constant* constZero = ConstantInt::get(Type::getInt32Ty(*context), 0);
  Constant* constArray =
      ConstantExpr::getInBoundsGetElementPtr(moduleName, constZero);

  Value* constPtr = ConstantExpr::getBitCast(
...
bool RAInstrumentation::runOnModule(Module& M) {
  ...
  std::string mIdentifier = pos > 0 ? M.getModuleIdentifier().substr(pos + 1)
  ...
  raw_fd_ostream File(Filename.data(), errorCode, sys::fs::F_None);

  if (!ErrorInfo.empty()) {
    errs() << "Error opening file /tmp/RAHashNames." << mIdentifier
  ...
  if (!Main) Main = M.getFunction("MAIN__");
    ...
    for (Function::iterator BBit = Fit->begin(), BBend = Fit->end();
      ...
      for (BasicBlock::iterator Iit = BBit->begin(); Iit != BBit->end();
        ...
        if (isValidInst(Iit) && !IsNotOriginal(*Iit)) {
          ...
          File << mIdentifier << "." << Fit->getName() << "."
               << cast<Value>(Iit)->getName() << " " << cast<Value>(Iit)
               << "\n";

          // Get the pointer to the instruction. Note that the same number was
          // saved in the Hash Dictionary and it will be used into the compiled
          // program.
          Value* vInstruction = cast<Value>(Iit);

          Constant* constInt = ConstantInt::get(Type::getInt32Ty(*context),
                                                (uint64_t)vInstruction);
          Value* constPtr = ConstantExpr::getIntToPtr(
              constInt, PointerType::getUnqual(Type::getInt8Ty(*context)));

          // Insert the arguments into a queue. The queue will be consumed when
          // the next instruction is not a PHI function.
          args.push_back(constPtr);
          args.push_back(Iit);
          ...
          if (!isa<PHINode>(nextInstruction)) {
            ...
            while (!args.empty()) {
              ...
              CallInst* callSetCurrentMinMax = CallInst::Create(
                  &setCurrentMinMax, setCurrentMinMaxArgs, "", nextInstruction);
...
void RAInstrumentation::MarkAsNotOriginal(Instruction& inst) {
  inst.setMetadata("new-inst", MDNode::get(*context, ArrayRef<Metadata*>()));
}
...
Function& RAInstrumentation::GetSetCurrentMinMaxFunction() {
  ...
  func = Function::Create(
      ...
      GlobalValue::ExternalLinkage, "setCurrentMinMax", module);
  return *func;
}
...
Function& RAInstrumentation::GetPrintHashFunction() {
  ...
  func = Function::Create(
      FunctionType::get(Type::getVoidTy(*context), args, false),
      GlobalValue::ExternalLinkage, "printHash", module);
  return *func;
}
