    ...
    if ((bi = dyn_cast<BranchInst>(ti))) {
        if (bi->isConditional()) {
            ...
            if (comparison) {
                ...
                for (User::const_op_iterator opit = comparison->op_begin(), opend = comparison->op_end();
                     opit != opend; ++opit) {
                    Value *operand = *opit;
...
void vSSA::insertSigmas(TerminatorInst *TI, Value *V) {
    ...
    for (SmallVectorImpl<PHINode *>::iterator vit = vssaphi_created.begin(), vend = vssaphi_created.end();
         vit != vend; ++vit) {
...
SmallVector<PHINode *, 25> vSSA::insertPhisForSigma(Value *V, PHINode *sigma) {
    ...
    for (std::set<BasicBlock *>::iterator DFit = DF_BB->second.begin(), DFend = DF_BB->second.end();
         DFit != DFend; ++DFit) {
...
void vSSA::renameUsesToPhi(Value *V, PHINode *phi) {
    ...
    for (SmallVectorImpl<PHINode *>::iterator vit = sigmasRenamed.begin(), vend = sigmasRenamed.end();
         vit != vend; ++vit) {
...
void vSSA::populatePhis(SmallVector<PHINode *, 25> &vssaphi_created, Value *V) {
    ...
    for (SmallVectorImpl<PHINode *>::iterator vit = vssaphi_created.begin(), vend = vssaphi_created.end();
        ...
        for (DenseMap<BasicBlock *, unsigned>::iterator mit = howManyTimesIsPred.begin(), mend = howManyTimesIsPred.end();
             mit != mend; ++mit) {
            unsigned count;
