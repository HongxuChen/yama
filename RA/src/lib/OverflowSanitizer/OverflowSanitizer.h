...
struct OverflowSanitizer : public ModulePass {
 private:
  ...
  std::map<BasicBlock*, LoopInfo*>
      loopExitBlocks;  // Maps a Loop Exit Block to its loop
 ...
 public:
  ...
  OverflowSanitizer()
      : ModulePass(ID),
        ...
        GVstderr(NULL),
        FPrintF(NULL),
        overflowMessagePtr(NULL),
        truncErrorMessagePtr(NULL),
        overflowMessagePtr2(NULL),
        truncErrorMessagePtr2(NULL),
        AbortF(NULL){};
