...
static cl::opt<bool, false> OnlySLE(
    "only-SLE",
    cl::desc("Only consider SLE, ULE, SGE and UGE as loop conditions."),
    cl::NotHidden);
...
// Table 2
STATISTIC(NumInstructionsBefore,
...
bool OverflowSanitizer::isValidInst(Instruction* I) {
  if (dyn_cast<OverflowingBinaryOperator>(I))
    ...
    return TruncInstrumentation && I->getType()->isIntegerTy() &&
           I->getOperand(0)->getType()->isIntegerTy();
  else if (dyn_cast<BitCastInst>(I))
    // Only do the instrumentation of the bitcast if the cast is to a lower size
    // datatype (semantically equivalent to a trunc)
    return TruncInstrumentation && I->getType()->isIntegerTy() &&
           I->getOperand(0)->getType()->isIntegerTy() &&
...
void llvm::OverflowSanitizer::lookForValuesToSafe(Instruction* I,
...
bool basicBlockSearch(BasicBlock* src, BasicBlock* dst,
  ...
  return (src2 == NULL ? false : basicBlockSearch(src2, dst, blocksToVisit,
                                                  visitedBlocks));
}

bool llvm::OverflowSanitizer::isReachable(BasicBlock* src, BasicBlock* dst) {
  if (loopBlocks.count(src) && loopBlocks.count(dst))
    if (loopBlocks[src] == loopBlocks[dst])  // if both blocks belong to the
                                             // same loop, one can rech the
                                             // otherand we don't need to do the
                                             // DFS
      return true;
...
BasicBlock* OverflowSanitizer::NewOverflowOccurrenceBlock(Instruction* I,
                                                          BasicBlock* NextBlock,
                                                          Value* messagePtr) {
  Constant* SourceFile = getSourceFile(I);
  Constant* LineNumber = getLineNumber(I);
  Constant* InstructionIdentifier =
      ConstantInt::get(Type::getInt32Ty(*context), (long)I);

  BasicBlock* result =
      BasicBlock::Create(*context, "", I->getParent()->getParent(), NextBlock);
...
int llvm::OverflowSanitizer::countInstructions() {
  ...
  for (Module::iterator Fit = module->begin(), Fend = module->end();
       Fit != Fend; Fit++) {
...
int llvm::OverflowSanitizer::countOverflowableInsts() {
  ...
  for (Module::iterator Fit = module->begin(), Fend = module->end();
    ...
    for (Function::iterator BBit = Fit->begin(), BBend = Fit->end();
         BBit != BBend; BBit++) {
      for (BasicBlock::iterator I = BBit->begin(), E = BBit->end(); I != E;
           I++) {
...
bool ::OverflowSanitizer::runOnModule(Module& M) {
  ...
  // Check the branch condition of each loop header. Mark the loop headers that
  // are reachable by Inputs.
  for (std::map<BasicBlock*, LoopInfo*>::iterator Bit = loopExitBlocks.begin(),
                                                  Bend = loopExitBlocks.end();
       Bit != Bend; Bit++) {
    if (BranchInst* BI = dyn_cast<BranchInst>((Bit->first)->getTerminator())) {
      if (BI->isConditional()) {
        Value* loopCondition = BI->getCondition();

        std::pair<GraphNode*, int> nearestDependency =
            depGraph.depGraph->getNearestDependency(loopCondition, inputValues,
                                                    false);
  ...
  if (InpDepLoops)
    AverageDependencyDistance = totalDistance / inpDepLoopConditions.size();

  // Verify the input-dependent loop headers that are controlled by SLE, SGE,
  // ULE, UGE ICmpInsts. Mark them as vulnerable
  for (std::list<Value*>::iterator Cit = inpDepLoopConditions.begin(),
                                   Cend = inpDepLoopConditions.end();
       Cit != Cend; Cit++) {
    if (OnlySLE) {
      // The ICmpInst has 2 operands: X comes from input. Y is updated inside
      // the loop. We must make sure that Y will never overflow.
  ...
  for (std::set<Instruction*>::iterator i = valuesToSafe.begin(),
                                        e = valuesToSafe.end();
       i != e; i++) {
    ...
    if (abortBlocks.count(F)) {
      ...
      // Create the basic block which the control flow goes to when the
      // assertion fail
...
void ::OverflowSanitizer::analyzeLoops() {
  ...
  for (Module::iterator Fit = module->begin(), Fend = module->end();
    ...
    for (Function::iterator BBit = Fit->begin(), BBend = Fit->end();
      ...
      // FIXME this is not the correct way to get the exit blocks. We are
      // getting all the blocks that
      ...
      if (CurrentLoop) {
        ...
        for (succ_iterator PI = succ_begin(BBit), E = succ_end(BBit); PI != E;
             ++PI) {
          BasicBlock* succBB = *PI;
...
Constant* OverflowSanitizer::strToLLVMConstant(std::string s) {
  ...
  for (unsigned int i = 0; i < vec.size(); i++) {
    cVec.push_back(llvm::ConstantInt::get(Type::getInt8Ty(*context),
                                          APInt(8, vec[i], false)));
  }
...
/*
 * Inserts global variable and function declarations, needed during the
 * instrumentation.
 */
void OverflowSanitizer::InsertGlobalDeclarations() {
  ...
  std::string message2(
      "Truncation with data loss occurred in %s, line %d. [%d]\n");
  std::string message3("(Suspected) Overflow occurred in %s, line %d. [%d]\n");
  std::string message4(
      "(Suspected) Truncation with data loss occurred in %s, line %d. [%d]\n");

  Constant* stringConstant = strToLLVMConstant(message1);
  GlobalVariable* messageStr = new GlobalVariable(
      *module, stringConstant->getType(), true,
      llvm::GlobalValue::InternalLinkage, stringConstant, "OverflowMessage");

  // Get the int8ptr to our message
  constZero = ConstantInt::get(Type::getInt32Ty(*context), 0);
  Constant* constArray =
      ConstantExpr::getInBoundsGetElementPtr(messageStr, constZero);
  overflowMessagePtr = ConstantExpr::getBitCast(
      constArray, PointerType::getUnqual(Type::getInt8Ty(*context)));
  ...
  truncErrorMessagePtr = ConstantExpr::getBitCast(
      constArray, PointerType::getUnqual(Type::getInt8Ty(*context)));
  ...
  // Get the int8ptr to our message
  constArray = ConstantExpr::getInBoundsGetElementPtr(messageStr, constZero);
  overflowMessagePtr2 = ConstantExpr::getBitCast(
      constArray, PointerType::getUnqual(Type::getInt8Ty(*context)));
  ...
  // Get the int8ptr to our message
  constArray = ConstantExpr::getInBoundsGetElementPtr(messageStr, constZero);
  truncErrorMessagePtr2 = ConstantExpr::getBitCast(
      constArray, PointerType::getUnqual(Type::getInt8Ty(*context)));
  ...
  if (GVstderr == NULL) {
    ...
    StructType* IO_marker_ty =
        StructType::create(*context, "struct._IO_marker");
    ...
    GVstderr = new GlobalVariable(*module, IO_FILE_PTR_ty, false,
                                  GlobalValue::ExternalLinkage, NULL, "stderr");
  } else {
  ...
  // Get the fprintf() function (takes an IO_FILE* and an i8* followed by
  // variadic parameters)
  FPrintF = module->getOrInsertFunction(
      "fprintf", FunctionType::get(Type::getVoidTy(*context), aRParams, true));
  ...
  // Before declaring the abort function, we need to check whether it is already
  // declared or not
  ...
  if (!alreadyDeclared) {
    AbortF = Function::Create(AbortFTy, GlobalValue::ExternalLinkage, "abort",
                              module);
  }
}
...
void OverflowSanitizer::insertInstrumentation(Instruction* I,
                                              BasicBlock* AbortBB,
                                              OvfPrediction Pred) {
  ...
  Value* messagePtr =
      (Pred == OvUnknown ? overflowMessagePtr : overflowMessagePtr2);
  ...
  switch (I->getOpcode()) {
    case Instruction::Add:
      ...
      if (isSigned) {
        ...
        positiveOp2 =
            new ICmpInst(nextInstruction, CmpInst::ICMP_SGE, op2, constZero);
        canCauseOverflow = BinaryOperator::Create(
            Instruction::And, positiveOp1, positiveOp2, "", nextInstruction);
        negativeResult =
            new ICmpInst(nextInstruction, CmpInst::ICMP_SLT, I, constZero);
        hasIntegerBug1 =
            BinaryOperator::Create(Instruction::And, canCauseOverflow,
        ...
        negativeOp2 =
            new ICmpInst(nextInstruction, CmpInst::ICMP_SLT, op2, constZero);
        canCauseOverflow = BinaryOperator::Create(
            Instruction::And, negativeOp1, negativeOp2, "", nextInstruction);
        positiveResult =
            new ICmpInst(nextInstruction, CmpInst::ICMP_SGE, I, constZero);
        hasIntegerBug2 =
            BinaryOperator::Create(Instruction::And, canCauseOverflow,
        ...
        // AND the results
        hasIntegerBug =
            BinaryOperator::Create(Instruction::Or, hasIntegerBug1,
                                   hasIntegerBug2, "", nextInstruction);
      } else {
        ...
        lessThanOp2 = new ICmpInst(nextInstruction, CmpInst::ICMP_ULT,
                                   dyn_cast<Value>(I), op2);
        hasIntegerBug = BinaryOperator::Create(
            Instruction::Or, lessThanOp1, lessThanOp2, "", nextInstruction);
      }
      break;
    ...
    case Instruction::Sub:
      ...
      if (isSigned) {
        ...
        positiveOp2 =
            new ICmpInst(nextInstruction, CmpInst::ICMP_SGE, op2, constZero);
        canCauseOverflow = BinaryOperator::Create(
            Instruction::And, negativeOp1, positiveOp2, "", nextInstruction);
        positiveResult =
            new ICmpInst(nextInstruction, CmpInst::ICMP_SGE, I, constZero);
        hasIntegerBug1 =
            BinaryOperator::Create(Instruction::And, canCauseOverflow,
        ...
        negativeOp2 =
            new ICmpInst(nextInstruction, CmpInst::ICMP_SLT, op2, constZero);
        canCauseOverflow = BinaryOperator::Create(
            Instruction::And, positiveOp1, negativeOp2, "", nextInstruction);
        negativeResult =
            new ICmpInst(nextInstruction, CmpInst::ICMP_SLT, I, constZero);
        hasIntegerBug2 =
            BinaryOperator::Create(Instruction::And, canCauseOverflow,
    ...
    case Instruction::Mul:
      ...
      //  If the result of the division is not equal the other operand, there is
      //  an overflow
      ...
      canCauseOverflow =
          new ICmpInst(nextInstruction, CmpInst::ICMP_NE, op1, constZero);
      ...
      newBB2 =
          BasicBlock::Create(*context, "", I->getParent()->getParent(), newBB);

      // Remove the unconditional branch created by splitBasicBlock, and insert
      // a conditional
      ...
      if (isSigned) {
        tmpValue =
            BinaryOperator::Create(Instruction::SDiv, I, op1, "", branch);
        hasIntegerBug = new ICmpInst(branch, CmpInst::ICMP_NE, tmpValue, op2);

      } else {
        tmpValue =
            BinaryOperator::Create(Instruction::UDiv, I, op1, "", branch);
        hasIntegerBug = new ICmpInst(branch, CmpInst::ICMP_NE, tmpValue, op2);
      }
      ...
      BranchInst::Create(NewOverflowOccurrenceBlock(I, NextBB, messagePtr),
                         newBB, hasIntegerBug, newBB2);

      break;

    case Instruction::Shl:

      // The result of a Shift Left must be positive if the first operand is
      // positive
      ...
      if (isSigned) {
        ...
        negativeResult =
            new ICmpInst(nextInstruction, CmpInst::ICMP_SLT, I, constZero);
        hasIntegerBug1 = BinaryOperator::Create(
            Instruction::And, positiveOp1, negativeResult, "", nextInstruction);

        negativeOp1 =
            new ICmpInst(nextInstruction, CmpInst::ICMP_SLT, op1, constZero);
        canCauseOverflow =
            new ICmpInst(nextInstruction, CmpInst::ICMP_NE, op2, constZero);
        hasIntegerBug1 =
            BinaryOperator::Create(Instruction::And, canCauseOverflow,
                                   negativeOp1, "", nextInstruction);
        ...
        // if the result is greater than the first operand, something has gone
        // wrong in the SHL
        hasIntegerBug =
            new ICmpInst(nextInstruction, CmpInst::ICMP_ULT, I, op1);
      }
      break;
    ...
    case Instruction::Trunc:
       ...
       * 		Cast the truncated value back to its original type and
       * check if the value remains equal
       */
      messagePtr =
          (Pred == OvUnknown ? truncErrorMessagePtr : truncErrorMessagePtr2);
      if (isSigned) {
        tmpValue = new SExtInst(I, op1->getType(), "", nextInstruction);
        hasIntegerBug =
            new ICmpInst(nextInstruction, CmpInst::ICMP_NE, tmpValue, op1);

      } else {
        tmpValue = CastInst::CreateZExtOrBitCast(I, op1->getType(), "",
                                                 nextInstruction);
  ...
  if (I->getOpcode() != Instruction::Mul) {
    ...
    // Remove the unconditional branch created by splitBasicBlock, and insert a
    // conditional
    // branch that correctly connects to newBB and assertfail
    branch = cast<BranchInst>(I->getParent()->getTerminator());
    branch->eraseFromParent();
    ...
    branch =
        BranchInst::Create(NewOverflowOccurrenceBlock(I, NextBB, messagePtr),
                           newBB, hasIntegerBug, I->getParent());
  }
}
...
Constant* OverflowSanitizer::getSourceFile(Instruction* I) {
  ...
  if (SourceFiles.count(File)) {
    ...
    GlobalVariable* sourceFileStr = new GlobalVariable(
        *module, stringConstant->getType(), true,
        llvm::GlobalValue::InternalLinkage, stringConstant, "SourceFile");

    constZero = ConstantInt::get(Type::getInt32Ty(*context), 0);

    // Get the int8ptr to our message
    Constant* constArray =
        ConstantExpr::getInBoundsGetElementPtr(sourceFileStr, constZero);
    Constant* sourceFilePtr = ConstantExpr::getBitCast(
        constArray, PointerType::getUnqual(Type::getInt8Ty(*context)));
...
static RegisterPass<OverflowSanitizer> Y("overflow-sanitizer",
                                         "Overflow Sanitizer Pass");
