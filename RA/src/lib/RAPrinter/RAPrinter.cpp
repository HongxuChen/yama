...
namespace {
    ...
    public:
        ...
        virtual bool runOnModule(Module &M) {
            ...
            std::size_t pos = M.getModuleIdentifier().rfind("/");
                ...
                for (Function::iterator BBit = Fit->begin(), BBend = Fit->end();
                    ...
                    for (BasicBlock::iterator Iit = BBit->begin(); Iit != BBit->end();
                        ...
                        if (isValidInst(I)) {
                            ...
                            File << mIdentifier << "." << Fit->getName().str() << "."
                            << cast<Value>(Iit)->getName() << " " << r.getLower() << " "
                            << r.getUpper() << "\n";
                        }
                    }
                }
...
static RegisterPass<RAPrinterInterProceduralCousot> Y(
        "ra-printer-inter-cousot",
        "RangeAnalysis printer (Interprocedural - Cousot).", false, false);
