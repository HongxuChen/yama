...
void llvm::InputValues::insertInInputDepValues(Value* V) {
  if (!inputValues.count(V)) inputValues.insert(V);
}
...
void llvm::InputValues::collectMainArguments() {
  ...
  if (!main) main = module->getFunction("MAIN__");

  if (main) {
    for (Function::arg_iterator Arg = main->arg_begin(), aEnd = main->arg_end();
         Arg != aEnd; Arg++) {
...
std::set<Value*> llvm::InputValues::getInputDepValues() { return inputValues; }
