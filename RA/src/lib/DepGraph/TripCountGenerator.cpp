...
static cl::opt<bool, false> usePericlesTripCount(
    "usePericlesTripCount",
    cl::desc("Use Pericles's heuristic to estimate trip count."),
    cl::NotHidden);

static cl::opt<bool, false> useHybridTripCount(
    "useHybridTripCount",
    cl::desc(
        "Use Hybrid heuristic (Vector + Pericles) to estimate trip count."),
    cl::NotHidden);

static cl::opt<bool, false> estimateMinimumTripCount(
    "estimateMinimumTripCount",
    cl::desc("Estimate the minimum trip count of the loops."), cl::NotHidden);

STATISTIC(NumVectorEstimatedTCs, "Number of Estimated Trip Counts by Vector");
STATISTIC(NumPericlesEstimatedTCs,
          "Number of Estimated Trip Counts by Pericles");
...
STATISTIC(NumUnknownConditionsIL,
          "Number of Interval Loops With Unknown TripCount");
STATISTIC(NumUnknownConditionsEL,
          "Number of Equality Loops With Unknown TripCount");
STATISTIC(NumUnknownConditionsOL,
          "Number of Other Loops With Unknown TripCount");

STATISTIC(NumIncompatibleOperandTypes,
          "Number of Loop Conditions With non-integer Operands");
...
Value* TripCountGenerator::generatePericlesEstimatedTripCount(
    BasicBlock* header, BasicBlock* entryBlock, Value* Op1, Value* Op2,
    CmpInst* CI) {
  bool isSigned = CI->isSigned();

  BasicBlock* GT =
      BasicBlock::Create(*context, "", header->getParent(), header);
  BasicBlock* LE =
      BasicBlock::Create(*context, "", header->getParent(), header);
  BasicBlock* PHI =
      BasicBlock::Create(*context, "", header->getParent(), header);
  ...
  if (Op1->getType() != Op2->getType()) {
    if (Op1->getType()->getIntegerBitWidth() >
        Op2->getType()->getIntegerBitWidth()) {
      // expand op2
      if (isSigned)
        Op2 = Builder.CreateSExt(Op2, Op1->getType(), "");
      else
        Op2 = Builder.CreateZExt(Op2, Op1->getType(), "");

    } else {
      // expand op1
      if (isSigned)
        Op1 = Builder.CreateSExt(Op1, Op2->getType(), "");
      else
        Op1 = Builder.CreateZExt(Op1, Op2->getType(), "");
    }
  }

  assert(Op1->getType() == Op2->getType() &&
         "Operands with different data types, even after adjust!");
  ...
  if (isSigned)
    EstimatedTripCount = Builder.CreateSExtOrBitCast(
        sub, Type::getInt64Ty(*context), "EstimatedTripCount");
  else
    EstimatedTripCount = Builder.CreateZExtOrBitCast(
        sub, Type::getInt64Ty(*context), "EstimatedTripCount");
...
Value* TripCountGenerator::getValueAtEntryPoint(Value* source,
  ...
  // Option 2: Sequence of redefinitions with PHI node in the loop header.
  // Return the incoming value from the entry block
    ...
    if (V) {
      if (PHINode* PHI = dyn_cast<PHINode>(V)) {
        if (PHI->getParent() == loopHeader) {
          Value* IncomingFromEntry =
              PHI->getIncomingValueForBlock(ln.entryBlocks[loopHeader]);
  ...
  // Option 3: Sequence of loads/stores in the same memory location. Create load
  // in the entry block and return the loaded value
  if (LoadInst* LI = dyn_cast<LoadInst>(source)) {
    InstToCopy = LI;
  }

  // Option 4: Cast Instruction (Bitcast, Zext, SExt, Trunc etc...): Propagate
  // search (The value theoretically is the same)
  ...
  if (InstToCopy) {
    ...
    ln.entryBlocks[loopHeader]->getInstList().insert(
        ln.entryBlocks[loopHeader]->getFirstInsertionPt(), NEW_INST);
...
ProgressVector* TripCountGenerator::joinVectors(ProgressVector* Vec1,
                                                ProgressVector* Vec2) {
  /*
...
ProgressVector* TripCountGenerator::generateConstantProgressVector(
    Value* source, BasicBlock* loopHeader) {
  LoopInfoEx& li = getAnalysis<LoopInfoEx>();
  LoopControllersDepGraph& lcd = getAnalysis<LoopControllersDepGraph>();
  ...
  std::set<std::stack<GraphNode*> > paths =
      depGraph->getAcyclicPathsInsideSCC(sourceNode, sourceNode);

  // When a value has no cycle (single-node SCC) we must create an artificial
  // cycle that
  ...
  for (std::set<std::stack<GraphNode*> >::iterator pIt = paths.begin();
       pIt != paths.end(); pIt++) {
    ...
    Value* currentVectorValue =
        currentPathVector->getUniqueValue(Type::getInt64Ty(*context));
...
void equalizeTypes(Value*& Op1, Value*& Op2, bool isSigned,
                   IRBuilder<> Builder) {
  if (Op1->getType() != Op2->getType()) {
    if (Op1->getType()->getIntegerBitWidth() >
        Op2->getType()->getIntegerBitWidth()) {
...
Instruction* TripCountGenerator::generateReplaceIfEqual(
  ...
  BasicBlock* EQ = BasicBlock::Create(
      *context, "", InsertBefore->getParent()->getParent(), endBB);
...
Instruction* llvm::TripCountGenerator::generateModuleOfSubtraction(
  ...
  BasicBlock* GT = BasicBlock::Create(
      *context, "", InsertBefore->getParent()->getParent(), endBB);
  BasicBlock* LE = BasicBlock::Create(
      *context, "", InsertBefore->getParent()->getParent(), endBB);
...
Value* llvm::TripCountGenerator::generateVectorEstimatedTripCount(
  ...
  Instruction* initialDistance =
      generateModuleOfSubtraction(Op1, Op2, isSigned, T);

  Instruction* step = generateModuleOfSubtraction(_V1, _V2, isSigned, T);

  // Preventing division by zero
  step = generateReplaceIfEqual(step, ConstantInt::get(step->getType(), 0),
                                ConstantInt::get(step->getType(), 1), T);
  ...
  if (isSigned) {
    ...
    EstimatedTripCount =
        BinaryOperator::CreateSDiv(initialDistance, step, "TC", T);
  } else {
    // We create an unsigned div
    EstimatedTripCount =
        BinaryOperator::CreateUDiv(initialDistance, step, "TC", T);
  }

  if (isSigned)
    EstimatedTripCount = CastInst::CreateSExtOrBitCast(
        EstimatedTripCount, Type::getInt64Ty(*context), "EstimatedTripCount",
        T);
  else
    EstimatedTripCount = CastInst::CreateSExtOrBitCast(
        EstimatedTripCount, Type::getInt64Ty(*context), "EstimatedTripCount",
  ...
  switch (CI->getPredicate()) {
    ...
    case CmpInst::ICMP_SLE: {
      Constant* One = ConstantInt::get(EstimatedTripCount->getType(), 1);
      EstimatedTripCount =
          BinaryOperator::CreateAdd(EstimatedTripCount, One, "", T);
      break;
    }
    default:
      break;
  ...
  // Adjust the PHINodes of the loop header accordingly
  //
  // This is necessary because one of the incoming blocks of the loop header has
  // changed
  for (BasicBlock::iterator it = header->begin(); it != header->end(); it++) {
    Instruction* tmp = it;
...
void llvm::TripCountGenerator::generateHybridEstimatedTripCounts(Function& F) {
  ...
  for (LoopInfoEx::iterator lit = li.begin(); lit != li.end(); lit++) {
    ...
    if (!CI)
      ...
      if ((!Op1) || (!Op2)) {
        if (!LoopClass)
          NumUnknownConditionsIL++;
        else
          NumUnknownConditionsEL++;
    ...
    if (!unknownTC) {
      generateVectorEstimatedTripCount(header, entryBlock, Op1, Op2, V1, V2,
                                       CI);
 ...
 * In repeat-until-loops, the loop controller is a basic block that has a
 *successor
...
BasicBlock* TripCountGenerator::findLoopControllerBlock(Loop* l) {
  ...
  for (SmallVectorImpl<BasicBlock*>::iterator It = exitBlocks.begin(),
                                              Iend = exitBlocks.end();
       It != Iend; It++) {
    BasicBlock* BB = *It;
    if (BB == header) {
  ...
  // Case 2: repeat-until/do-while (the exiting block can branch directly to the
  // header)
  for (SmallVectorImpl<BasicBlock*>::iterator It = exitBlocks.begin(),
                                              Iend = exitBlocks.end();
       It != Iend; It++) {
    ...
    // Here we iterate over the successors of BB to check if the header is one
    // of them
...
static RegisterPass<TripCountGenerator> X("tc-generator",
                                          "Trip Count Generator");
