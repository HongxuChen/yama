...
bool AliasSets::runOnModule(Module &M) {
  ...
  std::map<int, std::set<int> > allPointsTo =
      PA->allPointsTo();  // sets of alias represented as ints
                          /*
                           *   The sets represented by allPointsTo are not disjoint.
                           * 		Thus, we will process them and create disjoint sets that are useful
                           * for many other analyses.
                           */

  int count = 0;

  for (std::map<int, std::set<int> >::iterator i = allPointsTo.begin(),
                                               e = allPointsTo.end();
       i != e; ++i) {
    bool found = false;
    int disjointSetIndex;

    // First, we discover the disjoint set that contains at least one of those
    // values.
    // if none of the values are contained in any existing set, we create a new
    // one
    ...
    if (!found) {
      for (std::set<int>::iterator ii = i->second.begin(), ee = i->second.end();
           ii != ee; ++ii) {
  ...
  for (llvm::DenseMap<int, int>::iterator i = disjointSet.begin(),
                                          e = disjointSet.end();
       i != e; ++i) {
  ...
  for (llvm::DenseMap<int, std::set<int> >::iterator i = disjointSets.begin(),
                                                     e = disjointSets.end();
       i != e; ++i) {
...
void AliasSets::printSets() {
  ...
  for (llvm::DenseMap<int, std::set<Value *> >::iterator
           i = valueDisjointSets.begin(),
           e = valueDisjointSets.end();
       i != e; ++i) {
    errs() << "Set " << i->first << " :\n";
...
static RegisterPass<AliasSets> X("alias-sets",
                                 "Get alias sets from pointer analysis pass",
                                 false, false);
