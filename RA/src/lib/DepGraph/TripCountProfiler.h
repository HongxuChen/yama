...
class TripCountProfiler : public FunctionPass {
 public:
  ...
  TripCountProfiler()
      : FunctionPass(ID),
        formatStr(NULL),
        moduleIdentifierStr(NULL),
        context(NULL),
        initLoopList(NULL),
        collectLoopData(NULL),
        flushLoopStats(NULL){};
  ~TripCountProfiler(){};
  ...
  Value* generateEstimatedTripCount(BasicBlock* header, BasicBlock* entryBlock,
                                    Value* Op1, Value* Op2, CmpInst* CI);
  void saveTripCount(std::set<BasicBlock*> BB, AllocaInst* tripCountPtr,
                     Value* estimatedTripCount, BasicBlock* loopHeader,
                     int LoopClass);
