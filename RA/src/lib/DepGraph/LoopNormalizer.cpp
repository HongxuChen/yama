...
void LoopNormalizer::switchBranch(BasicBlock* FromBlock, BasicBlock* PreviousTo,
                                  BasicBlock* NewTo) {
  ...
  if (BranchInst* BI = dyn_cast<BranchInst>(T)) {
    if (BI->isUnconditional()) {
      ...
      // We iterate over the successors to find which one leads to our previous
      // successor
    ...
    assert(false &&
           "TerminatorInst neither is a BranchInst nor is a SwitchInst.");
  }
}

BasicBlock* LoopNormalizer::normalizePreHeaders(
    std::set<BasicBlock*> PreHeaders, BasicBlock* Header) {
  BasicBlock* EntryBlock = BasicBlock::Create(
      Header->getParent()->getContext(), "Entry", Header->getParent(), Header);

  // Unconditional branch from the entry block to the loop header
  BranchInst* br = BranchInst::Create(Header, EntryBlock);

  // Disconnect the PreHeaders from the Header and Connect them to the Entry
  // Block
  for (std::set<BasicBlock*>::iterator it = PreHeaders.begin(),
                                       iend = PreHeaders.end();
       it != iend; it++) {
  ...
  for (BasicBlock::iterator it = Header->begin(); it != Header->end(); it++) {
    if (PHINode* PHI = dyn_cast<PHINode>(it)) {
      ...
      for (unsigned int i = 0; i < PHI->getNumIncomingValues(); i++) {
        if (PreHeaders.count(PHI->getIncomingBlock(i)))
          OutsideIncomingValues.insert(i);
      }
      ...
      // More than one value: we must create a PHI in the EntryBlock and use its
      // value in the Header
      else if (numIncomingValues > 1) {
        PHINode* newPHI = PHINode::Create(PHI->getType(), numIncomingValues, "",
                                          EntryBlock->getTerminator());

        for (std::set<unsigned int>::iterator i = OutsideIncomingValues.begin();
             i != OutsideIncomingValues.end(); i++) {
...
void LoopNormalizer::normalizePostExit(BasicBlock* exitBlock,
                                       BasicBlock* postExitBlock) {
  if (!postExitBlock->isLandingPad()) {
    // LandingPads are already normalized

    BasicBlock* newBB =
        BasicBlock::Create(exitBlock->getParent()->getParent()->getContext(),
    ...
    for (BasicBlock::iterator i = postExitBlock->begin();
         i != postExitBlock->end(); i++) {
      Instruction* I = i;
 ...
 * A loop may have more than one post exit block, if it has more than one exit
 *point
...
bool llvm::LoopNormalizer::runOnFunction(Function& F) {
  ...
  for (LoopInfoEx::iterator it = li.begin(); it != li.end(); it++) {
    ...
    for (pred_iterator pred = pred_begin(header); pred != pred_end(header);
         pred++) {
  ...
  //	for (std::set<BasicBlock*>::iterator it = loopExits.begin(); it!=
  //loopExits.end(); it++){
  //
  //		BasicBlock* exitBlock = *it;
  //
  //		for(succ_iterator succ = succ_begin(exitBlock); succ !=
  //succ_end(exitBlock); succ++){
  //
  //			BasicBlock* successor = *succ;
  //
  //			if (li.getLoopDepth(successor) <
  //li.getLoopDepth(exitBlock)){
  //
...
static RegisterPass<LoopNormalizer> Y("loop-normalizer", "Loop Normalizer");
