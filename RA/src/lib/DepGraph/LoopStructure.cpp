...
STATISTIC(NumUnhandledExits,
          "Number of not handled exit points (not branch or switch)");
...
std::set<T> getDelta(std::set<T> t1, std::set<T> t2) {
  std::set<T> result;
  for (typename std::set<T>::iterator it = t1.begin(), iend = t1.end();
       it != iend; it++) {
    T item = *it;
    if (!t2.count(item)) {
      result.insert(item);
...
bool LoopStructure::runOnFunction(Function& F) {
  ...
  //	if (!ErrorInfo.empty()){
  //	  errs() << "Error opening file /tmp/DepGraphLoopPaths.txt for writing!
  ...
  for (LoopInfoEx::iterator it = li.begin(); it != li.end(); it++) {
    ...
    for (SmallVectorImpl<BasicBlock*>::iterator bIt = exitingBlocks.begin();
         bIt != exitingBlocks.end(); bIt++) {
      ...
      if (Condition) {
        if (GraphNode* ConditionNode = graph->findNode(Condition)) {
          ...
          graph->dfsVisitBack_ext(ConditionNode, visitedNodes,
                                  firstNodeVisitedPerSCC);

          std::set<GraphNode*> delta = getDelta(visitedNodes, tmp);

          // Iterate over the dependencies
          for (std::set<GraphNode*>::iterator nIt = delta.begin();
               nIt != delta.end(); nIt++) {
            ...
            if (graph->getSCC(SCCID).size() > 1) {
              ...
              if (!analyzedSCCs.count(SCCID)) {
                ...
                GraphNode* firstNodeVisitedInSCC =
                    firstNodeVisitedPerSCC[SCCID];

                std::set<std::stack<GraphNode*> > paths =
                    graph->getAcyclicPathsInsideSCC(firstNodeVisitedInSCC,
                                                    firstNodeVisitedInSCC);
