...
static cl::opt<bool, false> includeAllInstsInDepGraph(
    "includeAllInstsInDepGraph",
    cl::desc("Include All Instructions In DepGraph."), cl::NotHidden);
...
NodeClassId llvm::GraphNode::getClass_Id() const { return Class_ID; }

int llvm::GraphNode::getId() const { return ID; }
...
std::string llvm::GraphNode::getStyle() { return std::string("solid"); }
 ...
 * Class OpNode
 */
unsigned int OpNode::getOpCode() const { return OpCode; }
...
std::string llvm::OpNode::getShape() { return std::string("octagon"); }
...
GraphNode* llvm::OpNode::getIncomingNode(unsigned int index, edgeType et) {
  ...
  for (pred = predecessors.begin(), pred_end = predecessors.end();
       pred != pred_end && index >= 0; pred++) {
    if (pred->second == et) {
      if (index == 0) {
        result = pred->first;
      }
...
std::string llvm::PHIOpNode::getShape() { return std::string("octagon"); }
...
std::string llvm::PHIOpNode::getStyle() { return std::string("dashed"); }
...
std::string llvm::SigmaOpNode::getShape() { return std::string("octagon"); }
...
std::string llvm::SigmaOpNode::getStyle() { return std::string("dotted"); }
...
std::string llvm::CallNode::getShape() { return std::string("doubleoctagon"); }
...
CallInst* llvm::CallNode::getCallInst() const { return this->CI; }

/*
 * Class VarNode
 */
llvm::Value* VarNode::getValue() { return value; }
...
std::string llvm::MemNode::getShape() { return std::string("ellipse"); }
...
std::string llvm::MemNode::getStyle() { return std::string("dashed"); }

int llvm::MemNode::getAliasSetId() const { return aliasSetID; }

/*
 * Class DepGraph
 */
std::set<GraphNode*>::iterator DepGraph::begin() { return (nodes.begin()); }

std::set<GraphNode*>::iterator DepGraph::end() { return (nodes.end()); }

DepGraph::~DepGraph() { nodes.clear(); }

DepGraph* DepGraph::getParentGraph() { return parentGraph; }
...
DepGraph* DepGraph::clone() {
  ...
  result = new DepGraph(
      AS);  // Somebody has to free this memory at some point in the future
...
DepGraph DepGraph::makeSubGraph(std::set<GraphNode*> nodeList) {
  ...
  for (std::set<GraphNode*>::iterator it = nodeList.begin();
       it != nodeList.end(); ++it) {
    G.nodeMap[*it] = (*it)->clone();
  }

  // Copy the vertices
  for (std::map<GraphNode*, GraphNode*>::iterator it = G.nodeMap.begin();
       it != G.nodeMap.end(); ++it) {
    ...
    if (!G.nodes.count(it->second)) {
      ...
      if (isa<OpNode>(it->second)) {
        G.opNodes[dyn_cast<OpNode>(it->second)->getOperation()] =
...
void DepGraph::dfsVisitBack_ext(
    GraphNode* u, std::set<GraphNode*>& visitedNodes,
...
// Here we look for a back edge that leads to a node different than the first
// node.
bool lookForNestedLoop(GraphNode* first, GraphNode* current,
...
void DepGraph::toDot(std::string s) { this->toDot(s, &errs()); }
...
void llvm::DepGraph::toDot(std::string s, raw_ostream* stream,
                           llvm::DepGraph::Guider* g) {
...
bool DepGraph::isSigma(const PHINode* Phi) {
  if (Phi->getName().startswith(sigmaString) ||
      Phi->getMetadata(sigmaString) != 0 || Phi->getMetadata("Sigma") != 0)
    return true;
...
GraphNode* DepGraph::addInst(Value* v) {
  ...
  if (isValidInst(v)) {  // If is a data manipulator instruction
    ...
    if (Var == NULL ||
        (Var != NULL && Inst != NULL &&
         findOpNode(v) == NULL)) {  // If it has not processed yet
      ...
      if (Var == NULL) {
        ...
        if (hasVarNode) {
          if (StoreInst* SI = dyn_cast<StoreInst>(v))
            Var = addInst(SI->getOperand(1));  // We do this here because we
                                               // want to represent the store
      ...
      if (Inst) {
        ...
        if (CI) {
          ...
          if (isSigma(PHI))
            Op = new SigmaOpNode(PHI);
          else
            Op = new PHIOpNode(PHI);
        } else {
          Op = new OpNode(Inst);
        }
        opNodes[Inst] = Op;
...
// It verify if the instruction is valid for the dependence graph, i.e. just
...
std::pair<GraphNode*, int> llvm::DepGraph::getNearestDependency(
  ...
  if (GraphNode* startNode = findNode(sink)) {
    ...
    for (std::set<GraphNode*>::iterator Nit = nodes.begin(), Nend = nodes.end();
         Nit != Nend; Nit++) {
    ...
    while (workList.size()) {
      ...
      for (std::map<GraphNode*, edgeType>::iterator pred = preds.begin(),
                                                    pend = preds.end();
           pred != pend; pred++) {
        if (nodeColor[pred->first] ==
            0) {  // the node hasn't been processed yet
...
std::map<GraphNode*, std::vector<GraphNode*> >
llvm::DepGraph::getEveryDependency(llvm::Value* sink,
...
int llvm::DepGraph::getNumOpNodes() { return opNodes.size(); }

int llvm::DepGraph::getNumCallNodes() { return callNodes.size(); }

int llvm::DepGraph::getNumMemNodes() { return memNodes.size(); }

int llvm::DepGraph::getNumVarNodes() { return varNodes.size(); }
...
int llvm::DepGraph::getNumDataEdges() { return getNumEdges(etData); }

int llvm::DepGraph::getNumControlEdges() { return getNumEdges(etControl); }
...
void llvm::DepGraph::strongconnect(GraphNode* node,
                                   ...
                                   int& currentIndex, std::stack<GraphNode*>& S,
...
void llvm::DepGraph::printSCC(int SCCid, raw_ostream& OS) {
  ...
  for (std::set<GraphNode*>::iterator it = sCCs[SCCid].begin();
       it != sCCs[SCCid].end(); it++) {
    GraphNode* Node = *it;
    OS << "	" << Node->getLabel() << ": "
...
void llvm::DepGraph::dumpSCCs() {
  errs() << "\nSCCs\n";
  for (std::map<int, std::set<GraphNode*> >::iterator it = sCCs.begin();
       it != sCCs.end(); it++) {
    errs() << "SCC[" << it->first << "] : " << it->second.size() << " nodes\n";
  }

  errs() << "\nNodes\n";
  for (std::map<GraphNode*, int>::iterator it = reverseSCCMap.begin();
       it != reverseSCCMap.end(); it++) {
...
std::list<int> llvm::DepGraph::getSCCTopologicalOrder() {
  ...
  if (!topologicalOrderedSCCs.size()) {
    ...
    // For each SCC....
    for (std::map<int, std::set<GraphNode*> >::iterator it = localSCCs.begin();
         it != localSCCs.end(); it++) {
      ...
      for (std::set<GraphNode*>::iterator node = it->second.begin();
    ...
    assert(localSCCs.size() == dagSCC.size() &&
           "DAG of SCCs have a wrong number of nodes");
    ...
    while (dagSCC.size() > 0) {
      ...
      assert(currentNode >= 0 &&
             "DAG of SCCs have a cycle (it is not a valid DAG)!");
...
int llvm::DepGraph::getSCCID(GraphNode* node) {
  ...
  if (!reverseSCCMap.count(node)) {
    if (node)
      errs() << "Requesting SCC ID for invalid node: " << &node << "\n";
    else
      errs() << "Requesting SCC ID for null node pointer\n";
    return -1;
  }
...
std::set<GraphNode*> llvm::DepGraph::getSCC(int ID) {
  ...
  if (sCCs.count(ID))
    ...
    errs() << "Requesting SCC for invalid ID: " << ID << "\n";
...
void llvm::DepGraph::getAcyclicPaths_rec(
    GraphNode* dst, std::set<GraphNode*>& visitedNodes,
    std::stack<GraphNode*>& path, std::set<std::stack<GraphNode*> >& result,
    int SCCID) {
...
std::set<std::stack<GraphNode*> > llvm::DepGraph::getAcyclicPaths(
...
std::set<std::stack<GraphNode*> > llvm::DepGraph::getAcyclicPathsInsideSCC(
    GraphNode* src, GraphNode* dst) {
  std::set<std::stack<GraphNode*> > result;
...
bool DepGraph::acyclicPathExists(GraphNode* src, GraphNode* dst,
                                 std::set<GraphNode*> visitedNodes, int SCCID) {
  ...
  while (worklist.size()) {
    ...
    if (visitedNodes.count(current)) {
      // Item that have just been popped from the worklist and have already been
      // visited
...
void moduleDepGraph::matchParametersAndReturnValues(Function& F) {
  ...
  for (i = 0, argptr = F.arg_begin(), e = F.arg_end(); argptr != e;
    ...
    if (argNode != NULL) depGraph->addEdge(argPHI, argNode);
