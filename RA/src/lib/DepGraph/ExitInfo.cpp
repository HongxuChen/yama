  ...
  for (Module::iterator Fit = M.begin(); Fit != M.end(); Fit++) {
    ...
    for (Function::iterator BB = Fit->begin(); BB != Fit->end(); BB++) {
      ...
      if (dyn_cast<UnreachableInst>(T)) {
        ...
        for (BasicBlock::iterator Iit = BB->begin(), Iend = BB->end();
             Iit != Iend; Iit++) {
          if (CallInst* CI = dyn_cast<CallInst>(Iit)) {
            Function* calledfunction = CI->getCalledFunction();

            if (calledfunction == exitF || calledfunction == abortF)
              exitPoints.insert(CI);
          }
        }
...
char ExitInfo::ID = 0;
static RegisterPass<ExitInfo> Y("exit-info", "Exit Information");
