...
void LoopControllersDepGraph::setPerspective(BasicBlock* LoopHeader) {
  ...
  if (!LoopHeader) {
    ...
    /*
     * Perspective: Given a natural loop L, a perspective
     *LoopControllersDepGraph of L
    ...
    for (std::set<GraphNode*>::iterator node = nodesToRemove.begin();
         node != nodesToRemove.end(); node++) {
...
bool LoopControllersDepGraph::runOnFunction(Function& F) {
  ...
  for (LoopInfoEx::iterator lit = li.begin(), lend = li.end(); lit != lend;
       lit++) {
    Loop* l = *lit;
    ...
    for (SmallVectorImpl<BasicBlock*>::iterator BB = loopExitingBlocks.begin();
         BB != loopExitingBlocks.end(); BB++) {
      if (BranchInst* BI = dyn_cast<BranchInst>((*BB)->getTerminator())) {
        ...
        loopExitPredicates.insert(IBI->getAddress());
                     ...
                     dyn_cast<InvokeInst>((*BB)->getTerminator())) {
        loopExitPredicates.insert(II);
      }
    }
  }

  // Step 3: Make a list of graph nodes that represent the dependencies of the
  // loop controllers
  std::set<GraphNode*> visitedNodes;

  for (std::set<Value*>::iterator v = loopExitPredicates.begin();
       v != loopExitPredicates.end(); v++) {
    if (GraphNode* valueNode = depGraph->findNode(*v))
      depGraph->dfsVisitBack(valueNode, visitedNodes);
    else
      errs() << "Function : " << F.getName()
             << " - Value not found in the graph : " << **v << "\n";
  }

  // Step 4: Remove from the graph all the nodes that are not in the list of
  // dependencies
  std::set<GraphNode*> nodesToRemove;
  for (DepGraph::iterator node = depGraph->begin(); node != depGraph->end();
       node++) {
...
static RegisterPass<LoopControllersDepGraph> Z(
    "LoopControllersDepGraph", "Loop Controllers Dependence Graph", true, true);

char ViewLoopControllersDepGraphSCCs::ID = 0;
static RegisterPass<ViewLoopControllersDepGraphSCCs> X(
...
void ModuleLoopControllersDepGraph::setPerspective(BasicBlock* LoopHeader) {
  // TODO: Implement this
  assert(false &&
         "Method ModuleLoopControllersDepGraph::setPerspective not implemented "
         "yet");
}
...
bool ModuleLoopControllersDepGraph::runOnModule(Module& M) {
  ...
  for (Module::iterator Fit = M.begin(), Fend = M.end(); Fit != Fend; Fit++) {
    ...
    for (LoopInfoEx::iterator lit = li.begin(), lend = li.end(); lit != lend;
      ...
      for (SmallVectorImpl<BasicBlock*>::iterator BB =
               loopExitingBlocks.begin();
           BB != loopExitingBlocks.end(); BB++) {
        if (BranchInst* BI = dyn_cast<BranchInst>((*BB)->getTerminator())) {
          loopExitPredicates.insert(BI->getCondition());
        } else if (SwitchInst* SI =
...
static RegisterPass<ModuleLoopControllersDepGraph> Y(
