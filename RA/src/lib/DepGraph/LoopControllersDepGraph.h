...
class LoopControllersDepGraph : public FunctionPass {
 ...
 public:
  ...
  virtual ~LoopControllersDepGraph() { freePerspectiveGraph(); }
  ...
  void printPers() {
    if (fullGraph == depGraph)
      errs() << "Full!\n";
    else
      errs() << "Pers!	Full:" << fullGraph->getNumVarNodes()
             << "	Slice:" << depGraph->getNumVarNodes() << "\n";
  }
};
...
class ModuleLoopControllersDepGraph : public ModulePass {
 public:
  ...
  static char ID;  // Pass identification, replacement for typeid.
  ModuleLoopControllersDepGraph() : ModulePass(ID), depGraph(NULL) {}
  void getAnalysisUsage(AnalysisUsage& AU) const;
...
class ViewLoopControllersDepGraphSCCs : public FunctionPass {
 public:
  static char ID;  // Pass identification, replacement for typeid.
  ViewLoopControllersDepGraphSCCs() : FunctionPass(ID) {}
  ...
  bool runOnFunction(Function& F) {
    ...
    //			DisplayGraph(sys::Path(Filename), true,
    //GraphProgram::DOT);
    ...
    for (std::map<int, std::set<GraphNode*> >::iterator it = SCCs.begin();
      ...
      g->generateSubGraph(it->first)
          .toDot(F.getParent()->getModuleIdentifier(), Filename);
