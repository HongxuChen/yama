...
void initLoopList();
void collectLoopData(int64_t LoopHeaderBBPointer, int64_t tripCount,
                     int64_t estimatedTripCount, int LoopClass);
...
void printInOrder(LoopResults results, FILE* outStream, int LoopClass,
                  char* moduleIdentifier, int64_t ID) {
  for (LoopResults::iterator it = results.begin(); it != results.end(); it++) {
    fprintf(outStream, "TripCount %d %s.%" PRId64 " %" PRId64 " %d\n",
            LoopClass, moduleIdentifier, ID, it->first, it->second);
  }
}
...
void addInstance(LoopStats& Stats, int64_t tripCount,
                 int64_t estimatedTripCount) {
  int64_t GroupID;

  if (estimatedTripCount >= tripCount - 1 &&
      estimatedTripCount <= tripCount - 1)
    GroupID = 3;
...
void collectLoopData(int64_t LoopHeaderBBPointer, int64_t tripCount,
  ...
  if (!loops.count(LoopHeaderBBPointer))
    loops[LoopHeaderBBPointer].LoopClass = LoopClass;
...
void flushLoopStats(char* moduleIdentifier) {
  ...
  if (!outStream) {
    ...
    for (LoopList::iterator it = loops.begin(); it != loops.end(); it++) {
      printInOrder(it->second.T, outStream, it->second.LoopClass,
                   moduleIdentifier, it->first);
    }
