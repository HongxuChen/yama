...
class DepGraphSCCAnalysis : public FunctionPass {
 ...
 public:
  ...
  DepGraphSCCAnalysis() : FunctionPass(ID), g(NULL) {}
  ...
  bool runOnFunction(Function& F) {
    LoopControllersDepGraph& DepGraphPass =
        getAnalysis<LoopControllersDepGraph>();
    ...
    for (std::map<int, std::set<GraphNode*> >::iterator it = SCCs.begin();
      ...
      if (MaxNumFunctionsInSCC < SCCFunctions.size())
        MaxNumFunctionsInSCC += SCCFunctions.size();
...
class ModuleDepGraphSCCAnalysis : public ModulePass {
 ...
 public:
  ...
  ModuleDepGraphSCCAnalysis() : ModulePass(ID), g(NULL) {}
  ...
  bool runOnModule(Module& M) {
    ModuleLoopControllersDepGraph& DepGraphPass =
        getAnalysis<ModuleLoopControllersDepGraph>();
    DepGraph* g = DepGraphPass.depGraph;
...
static RegisterPass<ModuleDepGraphSCCAnalysis> Y(
    "module-depgraph-scc-analysis",
