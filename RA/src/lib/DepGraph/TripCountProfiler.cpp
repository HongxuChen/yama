...
Value* insertAdd(BasicBlock* BB, AllocaInst* A) {
  ...
  ConstantInt* One =
      ConstantInt::get(Type::getInt64Ty(BB->getParent()->getContext()), 1);
  Value* I = Builder.CreateAdd(L, One);
  Builder.CreateAlignedStore(I, A, 4);
  return I;
}

void llvm::TripCountProfiler::saveTripCount(std::set<BasicBlock*> BBs,
                                            AllocaInst* tripCountPtr,
                                            Value* estimatedTripCount,
                                            BasicBlock* loopHeader,
                                            int LoopClass) {
  for (std::set<BasicBlock*>::iterator it = BBs.begin(), end = BBs.end();
       it != end; it++) {
    BasicBlock* BB = *it;

    IRBuilder<> Builder(BB->getFirstInsertionPt());

    ConstantInt* loopIdentifier =
        ConstantInt::get(Type::getInt64Ty(*context), (int64_t)loopHeader);
    ConstantInt* loopClass =
        ConstantInt::get(Type::getInt32Ty(*context), (int64_t)LoopClass);
...
bool llvm::TripCountProfiler::doInitialization(Module& M) {
  ...
  args.push_back(Type::getInt32Ty(
      *context));  // LoopClass (0=Interval; 1=Equality; 2=Other)
...
Value* TripCountProfiler::getValueAtEntryPoint(Value* source,
                                               BasicBlock* loopHeader) {
...
BasicBlock* TripCountProfiler::findLoopControllerBlock(Loop* l) {
  ...
  for (SmallVectorImpl<BasicBlock*>::iterator It = exitBlocks.begin(),
    ...
    // Here we iterate over the successors of BB to check if it is a block that
    // leads the control
    // back to the header.
...
bool TripCountProfiler::runOnFunction(Function& F) {
  ...
  if (!moduleIdentifierStr) {
    moduleIdentifierStr = Builder.CreateGlobalStringPtr(
        F.getParent()->getModuleIdentifier(), "moduleIdentifierStr");
  }
  ...
  for (std::set<Instruction*>::iterator Iit = eI.exitPoints.begin(),
                                        Iend = eI.exitPoints.end();
       Iit != Iend; Iit++) {
    Instruction* I = *Iit;
  ...
  Constant* unknownTripCount =
      ConstantInt::get(Type::getInt64Ty(F.getContext()), -2);
  ...
  for (LoopInfoEx::iterator lit = li.begin(); lit != li.end(); lit++) {
    ...
    if (!CI) {
      ...
      if ((!Op1) || (!Op2)) {
      } else if ((!Op1->getType()->isIntegerTy()) ||
    ...
    if (estimatedTripCount) {
      ...
      /*
       * We will collect the actual trip count and the estimate trip count in
       * every
      ...
      for (SmallVectorImpl<BasicBlock*>::iterator eb = exitBlocks.begin();
           eb != exitBlocks.end(); eb++) {
         ...
         */
        if (!CurrentEB->isLandingPad()) blocksToInstrument.insert(CurrentEB);
      }
