    ...
    for (BasicBlock::iterator I = BB->begin(), E = BB->end(); I != E; ++I) {
      ...
      switch (I->getOpcode()) {
        ...
        case Instruction::GetElementPtr: {
          ...
          const PointerType *PoTy =
              cast<PointerType>(GEPI->getPointerOperandType());
              ...
              for (unsigned i = 0; i < values.size(); i++) {
                ...
                if (memoryBlocks.count(vv)) {
                  for (unsigned j = 0; j < memoryBlocks[vv].size(); j++) {
                    ...
                    for (User::op_iterator it = GEPI->idx_begin(),
                                           e = GEPI->idx_end();
                         it != e; ++it) {
                      if (i == 1) {
                  ...
                  if (memoryBlock.count(vv)) {
                    ...
                    if (memoryBlock.count(v2)) {
                      ...
                      for (User::op_iterator it = GEPI2->idx_begin(),
                                             e = GEPI2->idx_end();
                           it != e; ++it) {
                        if (i == 1) {
                          if (isa<ConstantInt>(*it))
            ...
            for (unsigned i = 0; i < n; i++) {
              ...
              if (phiValues.count(v)) {
                if (memoryBlocks.count(v)) {
                  ...
                  memoryBlocks[I].insert(memoryBlocks[I].end(),
                                         memoryBlocks[v].begin(),
                                         memoryBlocks[v].end());
                }
              } else {
                if (memoryBlock.count(v)) {
...
void PADriver::matchReturnValueWithReturnVariable(Function &F) {
  ...
  for (Value::use_iterator UI = F.use_begin(), E = F.use_end(); UI != E; ++UI) {
    ...
    for (std::set<Value *>::iterator it = retVals.begin(), E = retVals.end();
         it != E; ++it) {
...
int PADriver::getNewMemoryBlock() { return nextMemoryBlock++; }

// ============================= //

int PADriver::getNewInt() { return getNewMemoryBlock(); }
...
int PADriver::Value2Int(Value *v) {
  ...
  if (value2int.count(v)) return value2int[v];
...
static RegisterPass<PADriver> X("pa", "Pointer Analysis Driver Pass", true,
                                true);
