...
GenericGraphNode<T>::~GenericGraphNode() {
  for (typename std::map<GenericGraphNode<T>*, int>::iterator pred =
           predecessors.begin();
       pred != predecessors.end(); pred++) {
    (*pred).first->successors.erase(this);
  }

  for (typename std::map<GenericGraphNode<T>*, int>::iterator succ =
           successors.begin();
...
template <typename T>
void GenericGraph<T>::dfsVisit(GenericGraphNode<T>* u,
                               std::set<GenericGraphNode<T>*>& visitedNodes) {
  visitedNodes.insert(u);

  std::map<GenericGraphNode<T>*, int> succs = u->getSuccessors();

  for (typename std::map<GenericGraphNode<T>*, int>::iterator
           succ = succs.begin(),
           s_end = succs.end();
       succ != s_end; succ++) {
...
template <typename T>
void GenericGraph<T>::dfsVisitBack(
    GenericGraphNode<T>* u, std::set<GenericGraphNode<T>*>& visitedNodes) {
  visitedNodes.insert(u);
...
//
//        for (std::set<GenericGraphNode<T>*>::iterator node = nodes.begin(),
//        end = nodes.end(); node
//                        != end; node++) {
//
//                if (DefinedNodes.count(*node) == 0) {
//                        (*stream) << (*node)->getName() << "[shape=" <<
//                        (*node)->getShape()
//                                        << ",style=" << (*node)->getStyle() <<
//                                        ",label=\""
//                                        << (*node)->getLabel() << "\"]\n";
...
//                }
//
//                std::map<GenericGraphNode<T>*, int> succs =
//                (*node)->getSuccessors();
//
//                for (std::map<GenericGraphNode<T>*, int>::iterator succ =
//                succs.begin(),
...
template <typename T>
void GenericGraph<T>::addEdge(GenericGraphNode<T>* src,
                              GenericGraphNode<T>* dst, int weight) {
  nodes.insert(src);
  nodes.insert(dst);
...
GenericGraphNode<T>* GenericGraph<T>::findNode(T element) {
  if (nodeMap.count(element)) return nodeMap[element];
...
template <typename T>
std::set<GenericGraphNode<T>*> GenericGraph<T>::findNodes(
    std::set<T> elements) {
  std::set<GenericGraphNode<T>*> result;

  for (typename std::set<T>::iterator i = elements.begin(),
                                      end = elements.end();
       i != end; i++) {
...
std::list<GenericGraphNode<T>*> GenericGraph<T>::getNodesWithoutPredecessors() {
  ...
  for (typename std::set<GenericGraphNode<T>*>::iterator node = nodes.begin(),
                                                         end = nodes.end();
       node != end; node++) {
...
template <typename T>
void GenericGraph<T>::removeEdge(GenericGraphNode<T>* src,
                                 GenericGraphNode<T>* dst) {
  src->disconnect(dst);
}
