...
void PointerAnalysis::addAddr(int A, int B) {
  if (debug)
    std::cerr << "Adding Addr Constraint: " << A << " = &" << B << std::endl;
...
void PointerAnalysis::addBase(int A, int B) {
  if (debug)
    std::cerr << "Adding Base Constraint: " << A << " = " << B << std::endl;
...
void PointerAnalysis::addStore(int A, int B) {
  if (debug)
    std::cerr << "Adding Store Constraint: *" << A << " = " << B << std::endl;
...
void PointerAnalysis::addLoad(int A, int B) {
  if (debug)
    std::cerr << "Adding Load Constraint: " << A << " = *" << B << std::endl;
...
void PointerAnalysis::addEdge(int fromId, int toId) {
  if (debug)
    std::cerr << "Adding Edge from " << fromId << " to " << toId << std::endl;
...
void PointerAnalysis::addToPts(int pointed, int pointee) {
  if (debug)
    std::cerr << "Adding " << pointed << " to pts(" << pointee << ")"
              << std::endl;
...
void PointerAnalysis::cycleSearch(int source, int target) {
  ...
  if (debug)
    std::cerr << "Looking for path between " << source << " and " << target
              << std::endl;
  ...
  if (debug)
    std::cerr << "Starting with " << source << " on the queue" << std::endl;
  ;
  queue.push(source);
  ...
  while (!queue.empty()) {
    ...
    queue.pop();
    if (debug)
      std::cerr << "Popped " << current << " from the queue" << std::endl;
    ;
    ...
    for (n = from[current].begin(); n != from[current].end(); ++n) {
      ...
      if (visited.find(repN) == visited.end()) {
        if (debug)
          std::cerr << "Add " << repN << " to the queue and marking it visited"
                    << std::endl;
        queue.push(repN);
        visited.insert(repN);
        ...
        if (debug)
          std::cerr << "Mark " << current << " as antecessor of " << *n
                    << std::endl;
        origin[repN] = current;
      }
    }
...
void PointerAnalysis::removeCycles() {
  ...
  for (V = activeVertices.begin(); V != activeVertices.end();) {
    ...
    if (debug)
      std::cerr << " * Current nextIt (Before): " << *nextIt << std::endl;
    if (repr[*V] != *V) {
      merge(*V, repr[*V]);
    }
    V = nextIt;
    ...
    if (debug)
      std::cerr << " * Current nextIt (After): " << *nextIt << std::endl;
  }
...
void PointerAnalysis::visit(int Node, IntMap& Order, IntMap& Repr,
  ...
  for (w = from[Node].begin(); w != from[Node].end(); w++) {
    ...
    if (Curr.find(*w) == Curr.end()) {
      Repr[Node] =
          (Order[Repr[Node]] < Order[Repr[*w]]) ? Repr[Node] : Repr[*w];
    }
  }
...
void PointerAnalysis::merge(int id, int target) {
  if (debug)
    std::cerr << " - Merging " << id << " into " << target << std::endl;
  ...
  vertices[id] = target;
  if (debug)
    std::cerr << "Removing vertice " << id << " from active." << std::endl;
...
bool PointerAnalysis::comparePts(int a, int b) {
  if (pointsToSet[a].size() != pointsToSet[b].size()) return false;

  IntSet::iterator V;
  for (V = pointsToSet[a].begin(); V != pointsToSet[a].end(); V++) {
    if (pointsToSet[b].find(vertices[*V]) == pointsToSet[b].end()) return false;
  }
  return true;
}
...
void PointerAnalysis::solve(bool withCycleRemoval) {
  ...
  while (!WorkSet.empty()) {
    ...
    for (IntSet::iterator Z = from[Node].begin(); Z != from[Node].end(); Z++) {
      ...
      if (debug)
        std::cerr << " - Comparing pts of " << Node << " and " << ZVal
                  << std::endl;
      ...
      if (withCycleRemoval) {
        ...
        if (debug) {
          ...
          std::cerr << std::endl;
          std::cerr << "PTS == PTS ? "
                    << (pointsToSet[repZ] == pointsToSet[repN]) << std::endl;
          std::cerr << "ComparePTS : " << comparePts(repZ, repN) << std::endl;
          std::cerr << "Edge: [" << edge << "] (" << (R.find(edge) == R.end())
                    << ")" << std::endl;
        }
...
void PointerAnalysis::print() {
  ...
  for (v = vertices.begin(); v != vertices.end(); v++) {
    std::cout << v->first << " -> {";
    IntSet::iterator n;
    for (n = pointsToSet[v->first].begin(); n != pointsToSet[v->first].end();
         n++) {
      std::cout << *n << ", ";
    }
...
void PointerAnalysis::printDot(std::ostream& output, std::string graphName,
  ...
  // Print the Edges
  for (setIt = activeVertices.begin(); setIt != activeVertices.end(); setIt++) {
    for (setIt2 = from[*setIt].begin(); setIt2 != from[*setIt].end();
         setIt2++) {
...
int PointerAnalysis::getNumOfMertgedVertices() { return numMerged; }

// ============================================= //

int PointerAnalysis::getNumCallsRemove() { return numCallsRemove; }

// ============================================= //

int PointerAnalysis::getNumVertices() { return this->vertices.size(); }
