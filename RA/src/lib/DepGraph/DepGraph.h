 ...
 *              - It has a OpCode that is compatible with llvm::Instruction
 *OpCodes
 *              - It may or may not store a value, that is the variable defined
 *by the operation
 */
class OpNode : public GraphNode {
 ...
 public:
  OpNode(int OpCode) : GraphNode(), OpCode(OpCode), inst(NULL) {
    this->Class_ID = OpNodeId;
    NrOpNodes++;
  };
  OpNode(Instruction* i) : GraphNode(), OpCode(i->getOpcode()), inst(i) {
    this->Class_ID = OpNodeId;
    NrOpNodes++;
  };
  ~OpNode() { NrOpNodes--; };
  static inline bool classof(const GraphNode* N) {
    return N->getClass_Id() == OpNodeId || N->getClass_Id() == CallNodeId ||
  ...
  virtual GraphNode* getIncomingNode(unsigned int index = 0,
                                     edgeType et = etData);
...
class UnaryOpNode : public OpNode {
 ...
 public:
  UnaryOpNode(UnaryInstruction* UOP) : OpNode(UOP), UOP(UOP) {
 ...
 * Class SigmaOpNode
 *
 * This class represents operation nodes of llvm::PHINode instructions that are
 *sigma operations
 */
class SigmaOpNode : public OpNode {
 ...
 public:
  SigmaOpNode(PHINode* Sigma) : OpNode(Sigma), Sigma(Sigma) {
...
class PHIOpNode : public OpNode {
 ...
 public:
  PHIOpNode(PHINode* PHI) : OpNode(PHI), PHI(PHI) {
...
class BinaryOpNode : public OpNode {
 ...
 public:
  BinaryOpNode(BinaryOperator* BOP) : OpNode(BOP), BOP(BOP) {
...
class CallNode : public OpNode {
 ...
 public:
  CallNode(CallInst* CI) : OpNode(CI), CI(CI) { this->Class_ID = CallNodeId; };
  static inline bool classof(const GraphNode* N) {
...
class VarNode : public GraphNode {
 ...
 public:
  VarNode(Value* value) : GraphNode(), value(value) {
 ...
 *              - It provides a method to get access to all the Values contained
 *in the AliasSet
 */
class MemNode : public GraphNode {
 ...
 public:
  MemNode(int aliasSetID, AliasSets* AS) : aliasSetID(aliasSetID), AS(AS) {
...
class DepGraph {
 private:
  ...
  std::map<GraphNode*, GraphNode*>
      nodeMap;  // Correspondence of nodes between graphs
  ...
  bool isValidInst(const Value* v);  // Return true if the instruction is valid
                                     // for dependence graph construction
  bool isMemoryPointer(
      const Value* v);  // Return true if the value is a memory pointer
 ...
 public:
  ...
  DepGraph(AliasSets* AS) : parentGraph(NULL), AS(AS) {
    NrEdges = 0;
  };  // Constructor
  ...
  GraphNode* findNode(const Value* op);  // Return the pointer to the node or
                                         // NULL if it is not in the graph
  GraphNode* findNode(GraphNode* node);  // Return the pointer to the node or
                                         // NULL if it is not in the graph

  std::set<GraphNode*> findNodes(std::set<Value*> values);

  OpNode* findOpNode(const Value* op);  // Return the pointer to the node or
                                        // NULL if it is not in the graph
  ...
  DepGraph generateSubGraph(Value* src, Value* dst);  // Take a source value and
                                                      // a destination value and
                                                      // find a Connecting
                                                      // Subgraph from source to
                                                      // destination
  DepGraph generateSubGraph(GraphNode* src, GraphNode* dst);
  DepGraph generateSubGraph(
      int SCCID);  // Generate sub graph containing only the selected SCC

  void dfsVisit(GraphNode* u,
                std::set<GraphNode*>&
                    visitedNodes);  // Used by findConnectingSubgraph() method
  void dfsVisitBack(
      GraphNode* u,
      std::set<GraphNode*>&
          visitedNodes);  // Used by findConnectingSubgraph() method

  void dfsVisitBack_ext(GraphNode* u, std::set<GraphNode*>& visitedNodes,
   ...
   *
   * Given a sink, returns the nearest source in the graph and the distance to
   *the nearest source
   */
  ...
  void strongconnect(GraphNode* node, std::map<GraphNode*, int>& index,
                     std::map<GraphNode*, int>& lowlink, int& currentIndex,
                     std::stack<GraphNode*>& S, std::set<GraphNode*>& S2,
                     std::map<int, std::set<GraphNode*> >& SCCs);
  ...
  bool acyclicPathExists(GraphNode* src, GraphNode* dst,
                         std::set<GraphNode*> alreadyVisitedNodes, int SCCID);

  bool hasNestedLoop(int SCCID);
  bool hasNestedLoop(GraphNode* first);

  void getAcyclicPaths_rec(GraphNode* dst, std::set<GraphNode*>& visitedNodes,
                           std::stack<GraphNode*>& path,
                           std::set<std::stack<GraphNode*> >& result,
                           int SCCID);

  std::set<std::stack<GraphNode*> > getAcyclicPaths(GraphNode* src,
                                                    GraphNode* dst);
  std::set<std::stack<GraphNode*> > getAcyclicPathsInsideSCC(GraphNode* src,
                                                             GraphNode* dst);
};
...
class SCC_Iterator {
 ...
 public:
  SCC_Iterator(DepGraph* Graph, int SCCID) : Graph(Graph), SCCID(SCCID) {
    for (DepGraph::iterator node_it = Graph->begin(), node_end = Graph->end();
  ...
  bool hasNext() { return (currentIt != Graph->end()); }
  ...
  GraphNode* getNext() {
    ...
    for (DepGraph::iterator node_end = Graph->end(); currentIt != node_end;
         currentIt++) {
 ...
 *
 * Module pass that provides a context-insensitive interprocedural dependence
 *graph
 *
 */
